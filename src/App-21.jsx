...
async function generateAndDownloadPDF(text, filename="CrossBorder-CV.pdf", templateId="modern"){
  try{
    ...
    const S=tmpl.styles;
    const NAME_COLOR=rgb(...S.nameColor);
    const ACCENT=rgb(...S.accentColor);
    const HEADER_COLOR=rgb(...S.headerColor);
    const BLACK=rgb(0.05,0.05,0.05);
    const GRAY=rgb(0.35,0.35,0.35);
    const PW=595,PH=842,ML=50,MR=50,MT=50,MB=45;
    const CW=PW-ML-MR;
    let page=doc.addPage([PW,PH]);
    let y=PH-MT;
    const newPage=()=>{page=doc.addPage([PW,PH]);y=PH-MT;};
    const chk=(h)=>{if(y-h<MB)newPage();};
    const drawBlock=(txt,f,sz,x,maxW,lh,clr)=>{
      ...
      for(const w of words){
        ...
        if(f.widthOfTextAtSize(test,sz)>maxW&&line){
          chk(lh);page.drawText(line,{x,y,size:sz,font:f,color:clr});y-=lh;line=w;
        }else line=test;
      }
      if(line){chk(lh);page.drawText(line,{x,y,size:sz,font:f,color:clr});y-=lh;}
    };
    const drawRule=(clr,thick)=>{page.drawLine({start:{x:ML,y},end:{x:PW-MR,y},thickness:thick,color:clr});};
    const SECTION_KEYS=["PROFESSIONAL SUMMARY","CORE COMPETENCIES","SKILLS","SYSTEMS & TOOLS","SYSTEMS AND TOOLS","TECHNICAL SKILLS","PROFESSIONAL EXPERIENCE","WORK EXPERIENCE","EXPERIENCE","EMPLOYMENT","EARLY CAREER — EMERGING MARKETS EXPERIENCE","EARLY CAREER — EMERGING MARKETS","EARLY CAREER","EDUCATION & CERTIFICATIONS","EDUCATION AND CERTIFICATIONS","EDUCATION","CERTIFICATIONS","ADDITIONAL","AWARDS","VOLUNTEER","LANGUAGES","PROJECTS","PUBLICATIONS","ACHIEVEMENTS","REFERENCES","PROFILE","OBJECTIVE"];
    ...
    for(const key of SECTION_KEYS){
      ...
      norm=norm.replace(new RegExp(`(${esc})\\s+([A-Z][a-z]|[A-Z]{2}[a-z])`,"gi"),"$1\n$2");
      norm=norm.replace(new RegExp(`(${esc})\\s+(B\\.?Tech|B\\.?Sc|M\\.?Sc|MBA|Ph\\.?D)`,"gi"),"$1\n$2");
    }
    norm=norm.replace(/\s*[•·▪]\s*/g,"\n• ");
    norm=norm.replace(/\s{2,}•/g,"\n•");
    norm=norm.replace(/([a-z,])\s{2,}([A-Z])/g,"$1\n$2");
    norm=norm.replace(/\.\s{2,}([A-Z])/g,".\n$1");
    ...
    const DEGREE_RE=/^(B\.?Tech|B\.?Sc|M\.?Sc|MBA|Ph\.?D|B\.?A\b|M\.?A\b|Bachelor|Master|Doctor|Certificate|Diploma|Associate)/i;
    let isFirst=true;
    ...
    for(let i=0;i<rawLines.length;i++){
      ...
      const isSection=SECTION_KEYS.some(k=>UP===k.toUpperCase()||UP.startsWith(k.toUpperCase()));
      if(isFirst){
        isFirst=false;
        chk(30);
        drawBlock(line,bold,S.nameSize,ML,CW,S.nameSize+4,NAME_COLOR);
        y-=2;
        continue;
      }
      if(i<=2&&(line.includes("@")||line.match(/\d{3}[\s\-]\d{3}/)||line.toLowerCase().includes("linkedin"))){
        chk(14);
        drawBlock(line,font,8.5,ML,CW,13,GRAY);
        y-=4;
        continue;
      }
      if(isSection){
        y-=10;chk(20);
        drawRule(ACCENT,S.ruleThick);
        y-=10;
        drawBlock(UP,S.headerBold?bold:font,9.5,ML,CW,14,HEADER_COLOR);
        y-=4;prevWasHeader=true;
        continue;
      }
      const dateMatch=line.match(DATE_RE);
      const hasPipe=line.includes("|")||line.includes("–")||line.includes("—");
      const isJobLine=hasPipe&&(dateMatch||line.match(/\d{4}/));
      if(isJobLine){
        y-=prevWasHeader?2:10;chk(22);
        if(dateMatch){
          const dateStr=dateMatch[0].trim();
          const titlePart=line.slice(0,line.search(DATE_RE)).trim().replace(/\s+$/,"");
          const dateW=font.widthOfTextAtSize(dateStr,9);
          const rowY=y;
          drawBlock(titlePart,bold,10,ML,CW-dateW-12,16,BLACK);
          page.drawText(dateStr,{x:PW-MR-dateW,y:rowY,size:9,font,color:GRAY});
        }else{
          drawBlock(line,bold,10,ML,CW,16,BLACK);
        }
        prevWasHeader=false;continue;
      }
      if(line.startsWith("•")||line.startsWith("-")){
        const bt=line.replace(/^[•\-]\s*/,"");
        chk(14);
        page.drawText("•",{x:ML+5,y,size:8.5,font,color:GRAY});
        drawBlock(bt,font,9.5,ML+18,CW-18,14,BLACK);
        y-=1;prevWasHeader=false;continue;
      }
      if(DEGREE_RE.test(line)){
        ...
        if(parts.length>=2){
          ...
          chk(14);
          const rowY=y;
          drawBlock(degree,bold,9.5,ML,CW*0.65,14,BLACK);
          const instW=italic.widthOfTextAtSize(institution,9);
          page.drawText(institution,{x:PW-MR-instW,y:rowY,size:9,font:italic,color:GRAY});
          prevWasHeader=false;continue;
        }else{
          chk(14);drawBlock(line,bold,9.5,ML,CW,14,BLACK);prevWasHeader=false;continue;
        }
      }
      chk(14);
      drawBlock(line,font,9.5,ML,CW,14,BLACK);
      prevWasHeader=false;
    }
