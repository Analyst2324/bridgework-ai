    ...
    try{
      ...
      if(ext==="pdf"){
        ...
        for(let pi=1;pi<=pdf.numPages;pi++){
          ...
          let lines=[];let curLine=[];let curY=null;
          for(const item of items){
            const itemY=Math.round(item.transform[5]);
            const str=item.str;if(!str)continue;
            if(curY===null||Math.abs(itemY-curY)>3){
              if(curLine.length)lines.push({y:curY,text:curLine.join("")});
              curLine=[str];curY=itemY;
            }else{
              const last=curLine[curLine.length-1]||"";
              if(last&&!last.endsWith(" ")&&!str.startsWith(" "))curLine.push(" "+str);
              else curLine.push(str);
            }
          }
          if(curLine.length)lines.push({y:curY,text:curLine.join("")});
