...
function PDFDownloadButton({text,filename}){
  ...
  const downloadPDF=async()=>{
    ...
    try{
      ...
      const HDRS=["PROFESSIONAL SUMMARY","CORE COMPETENCIES","PROFESSIONAL EXPERIENCE","WORK EXPERIENCE","EARLY CAREER — EMERGING MARKETS EXPERIENCE","EARLY CAREER","EDUCATION & CERTIFICATIONS","EDUCATION","CERTIFICATIONS","SKILLS","SYSTEMS & TOOLS","ACHIEVEMENTS","REFERENCES","ADDITIONAL","KEY COMPETENCIES","TITLE TRANSLATIONS MADE","KEY UPGRADES"];
    ...
    if(!text){setStatus("error");setMsg("No text to download.");return;}
    ...
    try{
      ...
      const HDRS=["PROFESSIONAL SUMMARY","CORE COMPETENCIES","PROFESSIONAL EXPERIENCE","WORK EXPERIENCE","EARLY CAREER — EMERGING MARKETS EXPERIENCE","EARLY CAREER","EDUCATION & CERTIFICATIONS","EDUCATION","CERTIFICATIONS","SKILLS","SYSTEMS & TOOLS","ACHIEVEMENTS","REFERENCES"];
      const DTRE=/((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\w*\.?\s+\d{4}\s*[–—-]+\s*(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\w*\.?\s+\d{4}|(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\w*\.?\s+\d{4}\s*[–—-]+\s*Present)\s*$/i;
      const ch=[];let idx=0;
      ...
      for(const line of lines){
        ...
        if(idx===0){
          ch.push(new Paragraph({children:[new TextRun({text:line,bold:true,size:34,color:NAVY,font:"Calibri"})],spacing:{after:60},border:{bottom:{color:GOLD,style:BorderStyle.SINGLE,size:8,space:4}}}));
          idx++;continue;
        }
        if(idx<=2&&(line.includes("@")||line.match(/\d{3}/)||line.toLowerCase().includes("linkedin"))){
          ch.push(new Paragraph({children:[new TextRun({text:line,size:18,color:GRAY,font:"Calibri"})],spacing:{after:80}}));
          idx++;continue;
        }
        if(isHdr){
          ch.push(new Paragraph({children:[new TextRun({text:UP,bold:true,size:20,color:GOLD,font:"Calibri",allCaps:true})],spacing:{before:200,after:60},border:{bottom:{color:GOLD,style:BorderStyle.SINGLE,size:4,space:1}}}));
          idx++;continue;
        }
        if(isJob){
          const ds=dm?dm[0].trim():"";
          const tp=ds?line.slice(0,line.search(DTRE)).trim():line;
          ch.push(new Paragraph({children:[new TextRun({text:tp,bold:true,size:22,color:NAVY,font:"Calibri"}),
            ...(ds?[new TextRun({text:"\t"+ds,size:18,color:GRAY,font:"Calibri"})]:[])],
            spacing:{before:120,after:40},tabStops:[{type:"right",position:convertInchesToTwip(6.5)}]}));
          idx++;continue;
        }
        if(isBul){
          ch.push(new Paragraph({children:[new TextRun({text:line.replace(/^[•\-]\s*/,""),size:20,font:"Calibri"})],bullet:{level:0},spacing:{after:40}}));
          idx++;continue;
        }
        ch.push(new Paragraph({children:[new TextRun({text:line,size:20,font:"Calibri"})],spacing:{after:40}}));
        idx++;
      }

      const doc=new Document({sections:[{properties:{page:{margin:{top:convertInchesToTwip(0.75),bottom:convertInchesToTwip(0.75),left:convertInchesToTwip(0.75),right:convertInchesToTwip(0.75)}}},children:ch}]});
      const blob=await Packer.toBlob(doc);
      const url=URL.createObjectURL(blob);
      const a=document.createElement("a");
