...
async function generateAndDownloadPDF(text, filename="CrossBorder-CV.pdf", templateId="modern"){
  try{
    ...
    const addFooter=()=>{
      page.drawText("Generated by CrossBorder AI · getcrossborder.com",
        {x:ML,y:22,size:7,font,color:LGRAY});
    };
    ...
    const normalized=text
      ...
      .replace(new RegExp(`\\b(${HEADER_PATTERN})\\b`,"g"),"\n\n$1")
      // Force newline AFTER section headers — critical fix for "PROFESSIONAL EXPERIENCE Associate..."
      // If a header is immediately followed by non-header text on same line, split them
      .replace(new RegExp(`(${HEADER_PATTERN})\\s+([A-Z][a-z])`, "g"),"$1\n$2")
      .replace(new RegExp(`(${HEADER_PATTERN})\\s+([A-Z]{2,}[^\\n])`, "g"),"$1\n$2")
      ...
      .replace(/\.\s{2,}([A-Z])/g,".\n$1")
      // Split "EDUCATION & CERTIFICATIONS B.TECH..." — header immediately followed by degree
      .replace(/(EDUCATION[^\\n]*?)\s+(B\.?TECH|B\.?SC|M\.?SC|MBA|PHD|B\.?A\b|M\.?A\b)/gi,"$1\n$2");
...
async function generateAndDownloadWord(text, filename="CrossBorder-CV.docx"){
  try{
    ...
    // Footer paragraph
    paragraphs.push(new Paragraph({
      children:[new TextRun({text:"Generated by CrossBorder AI · getcrossborder.com",size:14,color:"AAAAAA",font:"Calibri",italics:true})],
      spacing:{before:400},
