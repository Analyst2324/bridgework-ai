...
function PDFDownloadButton({text,filename}){
  ...
  const downloadPDF=async()=>{
    ...
    try{
      ...
      const HDRS=["PROFESSIONAL SUMMARY","CORE COMPETENCIES","PROFESSIONAL EXPERIENCE","WORK EXPERIENCE","EARLY CAREER — EMERGING MARKETS EXPERIENCE","EARLY CAREER","EDUCATION & CERTIFICATIONS","EDUCATION","CERTIFICATIONS","SKILLS","SYSTEMS & TOOLS","ACHIEVEMENTS","REFERENCES","ADDITIONAL"];
      ...
      for(const line of lines){
        ...
        if(idx===0){
          chk(28);bk(line,bold,S.nameSize,ML,CW,S.nameSize+4,NAME_C);y-=2;idx++;pt="name";continue;
        }
        if(idx<=2&&(line.includes("@")||line.match(/\d{3}[\s\-]\d{3}/)||line.toLowerCase().includes("linkedin"))){
          chk(13);bk(line,font,8.5,ML,CW,13,GRAY);y-=2;idx++;pt="contact";continue;
        }
        if(isHdr){
          y-=pt==="name"||pt==="contact"?8:12;chk(22);
          page.drawLine({start:{x:ML,y:y+3},end:{x:PW-MR,y:y+3},thickness:S.ruleThick,color:ACCENT});
          y-=9;bk(UP,S.headerBold?bold:font,9.5,ML,CW,14,HDR_C);y-=3;idx++;pt="hdr";continue;
        }
        if(isJob){
          y-=pt==="hdr"?4:10;chk(20);
          if(dm){
            const ds=dm[0].trim();
            const tp=line.slice(0,line.search(DTRE)).trim();
            const dw=font.widthOfTextAtSize(ds,9);
            const ry=y;
            bk(tp,bold,10,ML,CW-dw-14,16,BLACK);
            page.drawText(ds,{x:PW-MR-dw,y:ry,size:9,font,color:GRAY});
          }else{bk(line,bold,10,ML,CW,16,BLACK);}
          y-=1;idx++;pt="job";continue;
        }
        if(isBul){
