...
async function generateAndDownloadWord(text, filename="CrossBorder-CV.docx"){
  try{
    // Load docx library from unpkg (browser UMD build)
    await new Promise((resolve,reject)=>{
    ...
    for(const line of lines){
      if(line.startsWith("###NAME:")){
        ...
        children.push(new Paragraph({
          children:[new TextRun({text:n,bold:true,size:36,color:NAVY,font:"Calibri"})],
          spacing:{after:60},
...
function PDFDownloadButton({text,filename}){
  ...
  return(
    <div style={{marginTop:16,background:"rgba(0,0,0,0.15)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:14,padding:"18px 20px"}}>
      ...
      <div style={{fontSize:10,color:C.gray,marginTop:8}}>PDF is ATS-safe · PDF is ATS-safe · Word opens in Google Docs or Microsoft Word</div>
    </div>
  );
}
