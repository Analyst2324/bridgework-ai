...
").map(l=>l.trim()).filter(Boolean);
      ...
      const p=(content,opts={})=>{
        ...
        const sz2=`<w:sz w:val="${sz}"/><w:szCs w:val="${sz}"/>`;
        const border=opts.border?`<w:pBdr><w:bottom w:val="single" w:sz="4" w:space="1" w:color="C8881A"/></w:pBdr>`:"";
        const borderTop=opts.borderTop?`<w:pBdr><w:bottom w:val="single" w:sz="8" w:space="4" w:color="C8881A"/></w:pBdr>`:"";
        ...
        return `<w:p><w:pPr>${spacing}${ind}${border}${borderTop}${bullet}</w:pPr><w:r><w:rPr>${bold}${color}${caps}${sz2}</w:rPr><w:t xml:space="preserve">${content}</w:t></w:r></w:p>`;
      };

      const twoCol=(left,right,boldLeft=true)=>{
        const lsz=`<w:sz w:val="22"/><w:szCs w:val="22"/>`;
        const rsz=`<w:sz w:val="18"/><w:szCs w:val="18"/>`;
        const lb=boldLeft?"<w:b/>":"";
        return `<w:p><w:pPr><w:spacing w:before="120" w:after="40"/><w:tabs><w:tab w:val="right" w:pos="9360"/></w:tabs></w:pPr><w:r><w:rPr>${lb}<w:color w:val="0D1B2A"/>${lsz}</w:rPr><w:t xml:space="preserve">${left}</w:t></w:r><w:r><w:rPr><w:color w:val="666666"/>${rsz}</w:rPr><w:t xml:space="preserve">&#9;${right}</w:t></w:r></w:p>`;
      };
      ...
      for(const line of lines){
        ...
        if(idx===0){
          body+=p(s,{sz:34,bold:true,color:"0D1B2A",borderTop:true,after:60});
          idx++;continue;
        }
        if(idx<=3&&(line.includes("@")||/\d{3}/.test(line)||line.toLowerCase().includes("linkedin"))){
          body+=p(s,{sz:18,color:"666666",after:100});
          idx++;continue;
        }
        if(isHdr){
          body+=p(san(UP),{sz:20,bold:true,color:"C8881A",caps:true,before:200,after:60,border:true});
          idx++;continue;
        }
        if(isJob){
          const ds=dm?dm[0].trim():"";
          const tp=ds?san(line.slice(0,line.search(DTRE)).trim()):s;
          if(ds) body+=twoCol(tp,san(ds));
          else body+=p(tp,{sz:22,bold:true,color:"0D1B2A",before:120,after:40});
          idx++;continue;
        }
        if(isBul){
          const bt=san(line.replace(/^[•-]\s*/,""));
          body+=p(bt,{sz:20,bullet:true,after:40,ind:360});
          idx++;continue;
        }
        body+=p(s,{sz:20,after:40});
        idx++;
      }

      // Build complete docx XML
...
${body}
      ...
      // Create docx zip manually using JSZip
      await new Promise((res,rej)=>{
      ...
      zip.file("word/_rels/document.xml.rels",`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/numbering" Target="numbering.xml"/></Relationships>`);
