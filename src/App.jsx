...
function PDFDownloadButton({text,filename}){
  ...
  const downloadPDF=async()=>{
    if(!text){setError("No text.");return;}
    setPdfState("loading");setError("");
    try{
      if(!window.PDFLib){
      ...
      const bk=(txt,f,sz,x,mw,lh,clr)=>{
        const words=clean(String(txt||"")).split(/\s+/).filter(Boolean);
        let cur="";
      ...
      const HDRS=["PROFESSIONAL SUMMARY","CORE COMPETENCIES","KEY COMPETENCIES","PROFESSIONAL EXPERIENCE","WORK EXPERIENCE","EARLY CAREER","EDUCATION & CERTIFICATIONS","EDUCATION","CERTIFICATIONS","SKILLS","SYSTEMS & TOOLS","ACHIEVEMENTS","REFERENCES","ADDITIONAL","TITLE TRANSLATIONS","KEY UPGRADES","SUMMARY"];
      ...
      for(const line of lines){
        ...
        const isBul=line.startsWith("\u2022")||line.startsWith("* ")||line.startsWith("- ");
        ...
        if(idx===0){chk(28);bk(line,bold,S.nameSize,ML,CW,S.nameSize+4,NAME_C);page.drawLine({start:{x:ML,y:y+2},end:{x:PW-MR,y:y+2},thickness:S.ruleThick*1.5,color:ACCENT});y-=6;idx++;pt="name";continue;}
        if(idx<=3&&(line.includes("@")||/\d{3}/.test(line)||line.toLowerCase().includes("linkedin"))){chk(13);bk(line,font,8.5,ML,CW,13,GRAY);idx++;pt="contact";continue;}
        if(isHdr){y-=(pt==="name"||pt==="contact")?6:12;chk(22);page.drawLine({start:{x:ML,y:y+3},end:{x:PW-MR,y:y+3},thickness:S.ruleThick,color:ACCENT});y-=9;bk(UP,S.headerBold?bold:font,9.5,ML,CW,14,HDR_C);y-=2;idx++;pt="hdr";continue;}
        if(isJob){y-=(pt==="hdr")?4:10;chk(20);if(dm){const ds=dm[0].trim();const tp=line.slice(0,line.search(DTRE)).trim().replace(/[,\s]+$/,"");const dw=font.widthOfTextAtSize(ds,9);const ry=y;bk(tp,bold,10,ML,CW-dw-12,16,BLACK);page.drawText(ds,{x:PW-MR-dw,y:ry,size:9,font,color:GRAY});if(y===ry)y-=16;}else{bk(line,bold,10,ML,CW,16,BLACK);}idx++;pt="job";continue;}
        if(isBul){const bt=line.replace(/^[\u2022\*\-]\s*/,"");chk(14);page.drawText("\u2022",{x:ML+4,y,size:8.5,font,color:GRAY});bk(bt,font,9.5,ML+16,CW-16,14,BLACK);y-=1;idx++;pt="bul";continue;}
    ...
    }catch(e){console.error(e);setPdfState("idle");setError("PDF: "+e.message);}
  };

  const downloadWord=async()=>{
    if(!text){setError("No text.");return;}
    setWordState("loading");setError("");
    try{
      // Build docx using pure JavaScript - no library needed
      const san=sanitize;
      const HDRS=["PROFESSIONAL SUMMARY","CORE COMPETENCIES","KEY COMPETENCIES","PROFESSIONAL EXPERIENCE","WORK EXPERIENCE","EARLY CAREER","EDUCATION & CERTIFICATIONS","EDUCATION","CERTIFICATIONS","SKILLS","SYSTEMS & TOOLS","ACHIEVEMENTS","REFERENCES","ADDITIONAL","TITLE TRANSLATIONS","KEY UPGRADES","SUMMARY"];
      const DTRE=/((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\w*\.?\s+\d{4}\s*[-]+\s*(?:(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\w*\.?\s+\d{4}|Present))\s*$/i;

      const r=(t,b,color,sz)=>`<w:r><w:rPr>${b?"<w:b/>":""}${color?`<w:color w:val="${color}"/>`:""}${sz?`<w:sz w:val="${sz*2}"/><w:szCs w:val="${sz*2}"/>`:"<w:sz w:val=\"20\"/><w:szCs w:val=\"20\"/>"}<w:rFonts w:ascii="Calibri" w:hAnsi="Calibri"/></w:rPr><w:t xml:space="preserve">${t}</w:t></w:r>`;
      const p=(content,before,after,border)=>`<w:p><w:pPr><w:spacing w:before="${before||0}" w:after="${after||80}"/>${border?'<w:pBdr><w:bottom w:val="single" w:sz="4" w:space="1" w:color="C8881A"/></w:pBdr>':""}</w:pPr>${content}</w:p>`;
      const bul=(content)=>`<w:p><w:pPr><w:spacing w:before="0" w:after="40"/><w:ind w:left="360" w:hanging="180"/></w:pPr><w:r><w:rPr><w:sz w:val="20"/><w:szCs w:val="20"/><w:rFonts w:ascii="Calibri" w:hAnsi="Calibri"/></w:rPr><w:t xml:space="preserve">${String.fromCharCode(8226)}  ${content}</w:t></w:r></w:p>`;
      const job=(left,right)=>`<w:p><w:pPr><w:spacing w:before="160" w:after="40"/><w:tabs><w:tab w:val="right" w:pos="9000"/></w:tabs></w:pPr>${r(left,true,"0D1B2A",11)}${right?r("\t"+right,false,"666666",9):""}</w:p>`;
      ...
      for(const line of lines){
        ...
        const isJob=!isHdr&&hasSep&&/\d{4}/.test(line)&&line.length<220;
        if(idx===0){body+=p(r(s,true,"0D1B2A",16),0,60,true);idx++;continue;}
        if(idx<=3&&(line.includes("@")||/\d{3}/.test(line)||line.toLowerCase().includes("linkedin"))){body+=p(r(s,false,"666666",9),0,100);idx++;continue;}
        if(isHdr){body+=p(r(san(UP),true,"C8881A",10),240,60,true);idx++;continue;}
        if(isJob){const ds=dm?san(dm[0].trim()):"";const tp=ds?san(line.slice(0,line.search(DTRE)).trim()):s;body+=job(tp,ds);idx++;continue;}
        if(isBul){body+=bul(san(line.replace(/^[\u2022\*\-]\s*/,"")));idx++;continue;}
        body+=p(r(s,false,"",10),0,40);idx++;
      }

      const doc=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body>${body}<w:sectPr><w:pgMar w:top="1080" w:right="1080" w:bottom="1080" w:left="1080"/></w:sectPr></w:body></w:document>`;
      ...
      const cat=(...a)=>{const len=a.reduce((s,x)=>s+x.length,0);const r=new Uint8Array(len);let o=0;for(const x of a){r.set(x,o);o+=x.length;}return r;};
      const crc=(buf)=>{
        const t=[];for(let i=0;i<256;i++){let v=i;for(let j=0;j<8;j++)v=v&1?0xEDB88320^(v>>>1):v>>>1;t[i]=v;}
        let c=0xFFFFFFFF;for(const b of new Uint8Array(buf))c=t[(c^b)&255]^(c>>>8);return(c^0xFFFFFFFF)>>>0;
      };
      ...
      for(const[name,content] of files){
        ...
        const db=te.encode(content);
        const cr=crc(db);
        const lh=cat(
          new Uint8Array([0x50,0x4B,3,4]),u16(20),u16(0),u16(0),
          u16(0),u16(0),u32(cr),u32(db.length),u32(db.length),
          u16(nb.length),u16(0),nb,db
        );
        entries.push({nb,db,cr,off,lh});
        off+=lh.length;
      }
      let cd=new Uint8Array(0);
      for(const e of entries){
        cd=cat(cd,cat(
          new Uint8Array([0x50,0x4B,1,2]),u16(20),u16(20),
          u16(0),u16(0),u16(0),u16(0),u32(e.cr),
          u32(e.db.length),u32(e.db.length),
          u16(e.nb.length),u16(0),u16(0),u16(0),u16(0),u32(0),u32(e.off),e.nb
        ));
      }
      const eocd=cat(
        new Uint8Array([0x50,0x4B,5,6]),u16(0),u16(0),
        u16(entries.length),u16(entries.length),
        u32(cd.length),u32(off),u16(0)
      );
      const zip=cat(...entries.map(e=>e.lh),cd,eocd);
      ...
      document.body.appendChild(a);a.click();
      setTimeout(()=>{try{document.body.removeChild(a);}catch{}URL.revokeObjectURL(url);},2000);
      setWordState("done");setTimeout(()=>setWordState("idle"),3000);
    }catch(e){console.error(e);setWordState("idle");setError("Word: "+e.message);}
  };
    ...
    <div style={{marginTop:16,background:"rgba(0,0,0,0.15)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:14,padding:"18px 20px"}}>
      ...
      <div style={{display:"flex",gap:9,flexWrap:"wrap"}}>
        <button className="btn btn-gold" style={{flex:1,justifyContent:"center"}}
          onClick={downloadPDF} disabled={pdfState==="loading"||wordState==="loading"}>
          {pdfState==="loading"?"⏳ PDF...":pdfState==="done"?"✅ PDF Done!":"📄 Download PDF"}
        </button>
        <button className="btn btn-ghost" style={{flex:1,justifyContent:"center"}}
          onClick={downloadWord} disabled={pdfState==="loading"||wordState==="loading"}>
          {wordState==="loading"?"⏳ Word...":wordState==="done"?"✅ Word Done!":"📝 Download Word"}
        </button>
      </div>
      {error&&<div style={{marginTop:8,fontSize:11,color:C.red,background:"rgba(231,76,60,0.08)",borderRadius:8,padding:"8px 12px",border:"1px solid rgba(231,76,60,0.2)"}}>{error}</div>}
