...
function ResumeModule({country,onComplete}){
  ...
  const extractTextFromFile=async(f)=>{
    ...
    try{
      let text="";
      if(ext==="pdf"){
        // Load pdfjs via script tag with worker
      ...
      if(!text.trim()){
        setExtractError("Could not extract text from this file. Please tap 'Paste Text' and paste your CV directly.");
...
function ATSModule({country,onScore}){
  ...
  const extractText=async(f)=>{
    ...
    try{
      ...
      if(!text.trim()){setExtractError("Could not extract text. Please tap 'Paste Text' and paste your CV directly.");setExtracting(false);return;}
