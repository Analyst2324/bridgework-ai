...
["gmail","yahoo","hotmail","outlook","icloud","aol","proton","mail","ymail","live","msn"];
  ...
  const extractTextFromFile=async(f)=>{
    ...
    try{
      ...
      if(ext==="pdf"){
        ...
        const mammothMod=await import("https://cdn.jsdelivr.net/npm/mammoth@1.6.0/mammoth.browser.min.js");
        const arrayBuffer=await f.arrayBuffer();
      ...
      if(!text.trim()){
        setExtractError("Could not extract text from this file. Try pasting your CV text instead.");
        setExtracting(false);return;
      }
      setResumeText(text);setExtracting(false);setStep(1);
    }catch(err){
      console.error(err);
      setExtractError("Could not read this file. Please try pasting your CV text directly.");
      setExtracting(false);
    }
  };
...
function ATSModule({country,onScore}){
  ...
  const [extractError,setExtractError]=useState("");
  const [fileName,setFileName]=useState("");
  const [uploadMode,setUploadMode]=useState("upload"); // upload | paste
  ...
  const extractText=async(f)=>{
    ...
    try{
      ...
      if(!text.trim()){setExtractError("Could not extract text. Try pasting instead.");setExtracting(false);return;}
      setResumeText(text);setExtracting(false);
    }catch(e){setExtractError("Could not read this file. Please paste your CV text below.");setExtracting(false);}
  };
      ...
      <div className="panel-body">
        ...
        {uploadMode==="paste"&&(
          <textarea className="textarea-field" style={{minHeight:140}}
            placeholder="Paste your full CV text here. Open in Word/Google Docs → Select All → Copy → Paste."
