...
async function generateAndDownloadWord(text, filename="CrossBorder-CV.docx"){
  try{
    ...
    const {Document,Packer,Paragraph,TextRun,BorderStyle,convertInchesToTwip,AlignmentType}=window.docx;
...
function PDFDownloadButton({text,filename}){
  ...
  const downloadPDF=async()=>{
    ...
    try{
      ...
      const {Document,Packer,Paragraph,TextRun,BorderStyle,convertInchesToTwip}=window.docx;
      const NAVY="0D1B2A",GOLD="C8881A",GRAY="666666";
      const lines=text.split("\n").map(l=>l.trim()).filter(Boolean);
