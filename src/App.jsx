...
async function generateAndDownloadPDF(text, filename="CrossBorder-CV.pdf", templateId="modern"){
  try{
    ...
    const rc=rgb(...S.ruleColor);

    const W=595,H=842,ML=50,MR=50,MT=55,MB=50;
    const maxW=W-ML-MR;
    const DARK=rgb(0.1,0.1,0.1);
    const MID=rgb(0.35,0.35,0.35);
    const LGRAY=rgb(0.75,0.75,0.75);
    // Use template colors for name/accent/header
    const NAVY=nc;
    const GOLD=ac;
    ...
    const addFooter=()=>{};
    const newPage=()=>{addFooter();page=doc.addPage([W,H]);y=H-MT;};
    const chk=(n)=>{if(y-n<MB+20)newPage();};

    const wrapDraw=(txt,f,size,x,mw,lh,color)=>{
      if(!txt||!txt.trim())return;
      const words=txt.trim().split(/\s+/);
      let line="";
      for(const w of words){
        const t=line?`${line} ${w}`:w;
        if(f.widthOfTextAtSize(t,size)>mw&&line){
          chk(lh+2);page.drawText(line,{x,y,size,font:f,color});y-=lh;line=w;
        }else line=t;
      }
      if(line){chk(lh+2);page.drawText(line,{x,y,size,font:f,color});y-=lh;}
    };

    // SMART TEXT NORMALIZER
    const HEADER_PATTERN="PROFESSIONAL SUMMARY|SUMMARY|CORE COMPETENCIES|KEY COMPETENCIES|SKILLS|TECHNICAL SKILLS|PROFESSIONAL EXPERIENCE|WORK EXPERIENCE|EXPERIENCE|EMPLOYMENT|EDUCATION & CERTIFICATIONS|EDUCATION AND CERTIFICATIONS|EDUCATION|CERTIFICATIONS|EARLY CAREER — EMERGING MARKETS|EARLY CAREER|ADDITIONAL|AWARDS|VOLUNTEER|LANGUAGES|PROJECTS|PUBLICATIONS|ACHIEVEMENTS|REFERENCES|PROFILE|OBJECTIVE|SYSTEMS & TOOLS|SYSTEMS AND TOOLS";

    const normalized=text
      .replace(/\r\n|\r/g,"\n")
      // Force newline BEFORE known section headers
      .replace(new RegExp(`(${HEADER_PATTERN})`,"g"),"\n\n$1")
      // Force newline AFTER section headers when immediately followed by job title or degree text
      .replace(new RegExp(`(${HEADER_PATTERN})\\s+([A-Z][a-z])`,"g"),"$1\n$2")
      .replace(new RegExp(`(${HEADER_PATTERN})\\s+([A-Z]{2,}[^A-Z])`,"g"),"$1\n$2")
      // Split degree from education header: "EDUCATION & CERTIFICATIONS B.TECH..."
      .replace(/(EDUCATION[^,\n]*?)\s+(B\.?Tech|B\.?Sc|M\.?Sc|MBA|Ph\.?D|B\.?A|M\.?A|Bachelor|Master|Doctor)/gi,"$1\n$2")
      // Also handle ALL CAPS degree: "EDUCATION & CERTIFICATIONS B.TECH (HONS)..."
      .replace(/(EDUCATION[^,\n]*?)\s+(B\.\s*TECH|BTECH|BSC|MSC|MBA|PHD)/gi,"$1\n$2")
      // Force newline before bullet characters
      .replace(/\s+[•·▪]\s*/g,"\n• ")
      .replace(/\s{2,}•\s*/g,"\n• ")
      // Split job title lines from following text
      .replace(/([a-z])\s{2,}([A-Z])/g,"$1\n$2")
      .replace(/\.\s{2,}([A-Z])/g,".\n$1");

    const rawLines=normalized.split("\n").map(l=>l.trim()).filter(l=>l);

    const SECTION_HEADERS=["PROFESSIONAL SUMMARY","SUMMARY","CORE COMPETENCIES","KEY COMPETENCIES","SKILLS","TECHNICAL SKILLS","PROFESSIONAL EXPERIENCE","WORK EXPERIENCE","EXPERIENCE","EMPLOYMENT","EDUCATION & CERTIFICATIONS","EDUCATION AND CERTIFICATIONS","EDUCATION","CERTIFICATIONS","CERTIFICATION","EARLY CAREER — EMERGING MARKETS","EARLY CAREER","ADDITIONAL","AWARDS","AWARD","VOLUNTEER","LANGUAGES","PROJECTS","PUBLICATIONS","ACHIEVEMENTS","REFERENCES","PROFILE","OBJECTIVE","SYSTEMS & TOOLS","SYSTEMS AND TOOLS"];
    ...
    for(const line of rawLines){
      ...
      // ── NAME (very first line) ──
      if(isFirst){
        isFirst=false;
        chk(30);
        // Draw name
        wrapDraw(line,S.nameBold?bold:font,S.nameSize,ML,maxW,S.nameSize+6,nc);
        y-=2;
        // Gold rule under name
        page.drawLine({start:{x:ML,y:y+8},end:{x:W-MR,y:y+8},thickness:S.ruleThick*1.5,color:rc});
        y-=8;
        lineIndex++;
        continue;
      }
      ...
      if(lineIndex<=3&&(line.includes("@")||/\d{3}[\s\-\.]\d{3}/.test(line)||line.toLowerCase().includes("linkedin")||line.toLowerCase().includes("http"))){
        chk(14);
        wrapDraw(line,font,8.5,ML,maxW,13,MID);
        y-=1;
        lineIndex++;
        continue;
      }
      ...
      const isHeader=SECTION_HEADERS.some(h=>upperLine===h||upperLine.startsWith(h+"—")||upperLine.startsWith(h+" —")||upperLine.startsWith(h+"&")||upperLine.startsWith(h+" &"))||(line===line.toUpperCase()&&line.replace(/[^A-Za-z]/g,"").length>4&&!line.startsWith("•")&&!line.match(/^\d/)&&!line.match(/\d{4}/)&&line.length<80);
      if(isHeader){
        y-=14; // more space before section header
        chk(26);
        page.drawLine({start:{x:ML,y:y+6},end:{x:W-MR,y:y+6},thickness:S.ruleThick,color:rc});
        y-=6;
        wrapDraw(line.toUpperCase(),S.headerBold?bold:font,9.5,ML,maxW,15,hc);
        y-=8; // space AFTER header before first job entry
        lineIndex++;
        continue;
      }

      // ── JOB TITLE / COMPANY LINE ──
      const hasYear=line.match(/\d{4}/);
      const hasSeparator=line.includes("–")||line.includes("—")||line.includes("|")||line.includes("·");
      const hasMonth=line.match(/(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{4}/i);
      const isJobLine=(hasYear&&hasSeparator)||(hasMonth)||(hasSeparator&&!line.match(/^[A-Z\s&—–]+$/)&&line.length<120);
      if(isJobLine&&!isHeader){
        y-=10;
        chk(20);
        // Try to extract date from end of line e.g. "Jan 2022 – Jan 2024" or "Jan 2024 – Present"
        const dateMatch=line.match(/((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{4}\s*[–—-]\s*(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{4}|(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{4}\s*[–—-]\s*Present|\d{4}\s*[–—-]\s*(?:Present|\d{4}))$/i);
        if(dateMatch){
          const dateStr=dateMatch[0].trim();
          const titlePart=line.slice(0,line.lastIndexOf(dateStr)).trim().replace(/[,\s]+$/,"");
          const dateW=font.widthOfTextAtSize(dateStr,9);
          const titleMaxW=maxW-dateW-8;
          // Draw title bold on left
          wrapDraw(titlePart,bold,10,ML,titleMaxW,16,NAVY);
          // Draw date right-aligned on same y (need to go back up one line)
          y+=16;
          page.drawText(dateStr,{x:W-MR-dateW,y,size:9,font,color:MID});
          y-=16;
        } else {
          // No date found — just draw full line bold
          wrapDraw(line,bold,10,ML,maxW,16,NAVY);
        }
        y-=2;
        lineIndex++;
        continue;
      ...
      if(line.startsWith("•")||line.startsWith("-")){
        ...
        chk(15);
        page.drawText("•",{x:ML+4,y,size:9,font,color:MID});
        wrapDraw(bt,font,9.5,ML+18,maxW-18,14,DARK);
        y-=2; // small gap between bullets
        lineIndex++;
        continue;
      }

      // ── SKILLS / COMPETENCY CHIPS LINE ──
      if((line.match(/•/g)||[]).length>=2||(line.match(/\|/g)||[]).length>=2){
        chk(14);
        const cleaned=line.replace(/•/g," · ").replace(/\s{2,}/g," ");
        wrapDraw(cleaned,font,9.5,ML,maxW,14,MID);
        lineIndex++;
        continue;
      }

      // ── REGULAR TEXT — check if it's a degree/institution pair ──
      // Detect lines like "B.Tech (Hons), Pure and Applied Chemistry  Ladoke Akintola University"
      const isDegree=/^(B\.?Tech|B\.?Sc|M\.?Sc|MBA|Ph\.?D|B\.?A|M\.?A|Bachelor|Master|Doctor|Certificate|Diploma|Associate)/i.test(line);
      chk(14);
      if(isDegree){
        wrapDraw(line,bold,9.5,ML,maxW,14,DARK);
      } else {
        wrapDraw(line,font,9.5,ML,maxW,14,DARK);
      }
      lineIndex++;
    }
