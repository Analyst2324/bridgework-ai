      ...
      <div className="panel-body">
        ...
        {step===0&&(
          <div className="upload-zone" onClick={()=>inputRef.current.click()}
            ...
            <input ref={inputRef} type="file" accept=".txt,.doc,.docx,.pdf" style={{display:"none"}} onChange={e=>handleFile(e.target.files[0])}/>
            <div className="upload-icon">📤</div>
            <div className="upload-title">Drop your {country.docName.toLowerCase()} here</div>
            <div className="upload-sub">or click to browse</div>
            <div className="upload-formats">PDF · DOC · DOCX · TXT</div>
          </div>
        )}
...
function InterviewGame({country}){
  ...
  const submitAnswer=async()=>{
    ...
    if(currentQ+1>=TOTAL_Q){generateResult(updated);}
          ...
          {!textMode?(
            <div className="voice-status">
              ...
              <div className="voice-actions">
                ...
                <button className="btn btn-gold" onClick={submitAnswer} disabled={!transcript.trim()}>{currentQ+1>=TOTAL_Q?"Submit Final Answer →":"Next Question →"}</button>
              </div>
            </div>
          ):(
            <div>
              ...
              <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                ...
                <button className="btn btn-gold" onClick={submitAnswer} disabled={!textInput.trim()}>{currentQ+1>=TOTAL_Q?"Submit Final Answer →":"Next Question →"}</button>
              </div>
            </div>
          )}
        </div>
    ...
    <>
      ...
      <div className="app">
        <aside className="sidebar">
          <div className="logo"><div className="logo-mark">BridgeWork AI</div><div className="logo-sub">Your Global Career Partner</div></div>
            ...
            {active==="home"&&(
              <>
                ...
                <div style={{background:"rgba(232,168,56,0.05)",border:"1px solid rgba(232,168,56,0.12)",borderRadius:13,padding:"18px 22px"}}>
                  ...
                  <div style={{fontSize:13,color:C.cream,lineHeight:1.85}}>{country.titleGapExample} Yet most African professionals apply for Analyst roles. <strong>BridgeWork closes that gap.</strong></div>
                </div>
              </>
            )}
