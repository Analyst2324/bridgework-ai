...
async function generateAndDownloadPDF(text, filename="CrossBorder-CV.pdf", templateId="modern"){
  try{
    ...
    a.setAttribute("download",filename.replace(".docx",".doc"));
  ...
  try{
    ...
    for(const line of lines){
      if(line.startsWith("###NAME:")){
        ...
        rows.push(`<p style="font-size:18pt;font-weight:bold;color:#0D1B2A;margin:0 0 4px;font-family:Calibri,Arial;">${n}</p>`);
      }else if(line.startsWith("###CONTACT:")){
        const c=line.replace("###CONTACT:","").trim();
        rows.push(`<p style="font-size:9pt;color:#555;margin:0 0 10px;font-family:Calibri,Arial;">${c}</p>`);
      }else if(line.startsWith("###SECTION:")){
        const h=line.replace("###SECTION:","").trim();
        rows.push(`<p style="font-size:10pt;font-weight:bold;color:#C8881A;text-transform:uppercase;border-bottom:1px solid #C8881A;margin:12px 0 4px;padding-bottom:2px;font-family:Calibri,Arial;">${h}</p>`);
      }else if(line.startsWith("###JOB:")){
        const j=line.replace("###JOB:","").trim();
        const sep=j.indexOf("|||");
        const title=sep>=0?j.slice(0,sep).trim():j;
        const date=sep>=0?j.slice(sep+3).trim():"";
        rows.push(`<table width="100%" style="border:none;margin:8px 0 2px;"><tr><td style="font-size:10pt;font-weight:bold;font-family:Calibri,Arial;">${title}</td><td align="right" style="font-size:9pt;color:#666;font-family:Calibri,Arial;white-space:nowrap;">${date}</td></tr></table>`);
      }else if(line.startsWith("###BULLET:")){
        const b=line.replace("###BULLET:","").trim();
        rows.push(`<p style="font-size:9.5pt;margin:2px 0 2px 16px;font-family:Calibri,Arial;">• ${b}</p>`);
      }else if(line.startsWith("###BODY:")){
        const b=line.replace("###BODY:","").trim();
        rows.push(`<p style="font-size:9.5pt;margin:2px 0;line-height:1.5;font-family:Calibri,Arial;">${b}</p>`);
      }else if(line.startsWith("###EDU:")){
        const e=line.replace("###EDU:","").trim();
        const sep=e.indexOf("|||");
        const deg=sep>=0?e.slice(0,sep).trim():e;
        const inst=sep>=0?e.slice(sep+3).trim():"";
        rows.push(`<table width="100%" style="border:none;margin:3px 0;"><tr><td style="font-size:9.5pt;font-weight:bold;font-family:Calibri,Arial;">${deg}</td><td align="right" style="font-size:9pt;color:#555;font-style:italic;font-family:Calibri,Arial;">${inst}</td></tr></table>`);
      }
    }
    const html=`<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word" xmlns="http://www.w3.org/TR/REC-html40"><head><meta charset="UTF-8"></head><body style="margin:1in;">${rows.join("")}</body></html>`;
...
function PDFDownloadButton({text,filename}){
  ...
  const download=async(type)=>{
    if(!text||!text.trim()){setError("No resume text found. Please transform your CV first.");return;}
    setDownloading(type);setDone(null);setError("");
    try{
      let ok=false;
      if(type==="pdf") ok=await generateAndDownloadPDF(text,`${baseName}.pdf`,template);
      else ok=await generateAndDownloadWord(text,`${baseName}.doc`);
      if(ok){setDone(type);setTimeout(()=>setDone(null),3000);}
      else setError("Download failed — please try the other format.");
    }catch(e){
      console.error("Download error:",e);
      setError(`Error: ${e.message||"Download failed"}. Try the other format.`);
    }
    setDownloading(null);
  };
    ...
    <div style={{marginTop:16,background:"rgba(0,0,0,0.15)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:14,padding:"18px 20px"}}>
      ...
      <div style={{display:"flex",gap:9,flexWrap:"wrap"}}>
        <button className="btn btn-gold" style={{flex:1,justifyContent:"center",minWidth:120}}
          onClick={()=>download("pdf")} disabled={!!downloading}>
        ...
        <button className="btn btn-ghost" style={{flex:1,justifyContent:"center",minWidth:120}}
          onClick={()=>download("word")} disabled={!!downloading}>
          {downloading==="word"?"⏳ Generating...":done==="word"?"✅ Downloaded!":"📝 Download Word (.doc)"}
        </button>
      </div>
      {error&&<div style={{marginTop:8,fontSize:11,color:C.red,padding:"8px 12px",background:"rgba(231,76,60,0.08)",borderRadius:8,border:"1px solid rgba(231,76,60,0.2)"}}>{error}</div>}
