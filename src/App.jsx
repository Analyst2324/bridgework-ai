...
async function generateAndDownloadPDF(text, filename="CrossBorder-CV.pdf", templateId="modern"){
  try{
    ...
    // Check if output has markers — if yes use structured parser, else fallback
    const hasMarkers=text.includes("###NAME:")||text.includes("###SECTION:")||text.includes("###JOB:");
    ...
    if(hasMarkers){
      ...
      for(const line of lines){
        if(line.startsWith("###NAME:")){
          ...
          const hdr=line.replace("###SECTION:","").trim().toUpperCase();
          ...
          chk(14);
          const rowY=y;
          block(degree,bold,9.5,ML,CW*0.62,14,BLACK);
          if(inst){
            const instW=italic.widthOfTextAtSize(inst,9);
            page.drawText(inst,{x:PW-MR-instW,y:rowY,size:9,font:italic,color:MGRAY});
          }
          prevType="edu";

        }else if(line.startsWith("###TITLES:")||line.startsWith("###")){
          // Skip metadata lines
          continue;
        }else if(line.trim()){
          // Non-marker text — render as body
          chk(14);
      ...
      // ── FALLBACK: Plain text parser (best effort) ──
      // Used when AI doesn't follow marker format
      const KNOWN_HEADERS=["PROFESSIONAL SUMMARY","CORE COMPETENCIES","SYSTEMS & TOOLS","TECHNICAL SKILLS",
        "PROFESSIONAL EXPERIENCE","WORK EXPERIENCE","EARLY CAREER — EMERGING MARKETS EXPERIENCE",
        "EARLY CAREER — EMERGING MARKETS","EARLY CAREER","EDUCATION & CERTIFICATIONS",
        "EDUCATION AND CERTIFICATIONS","EDUCATION","CERTIFICATIONS","ADDITIONAL","AWARDS",
        "VOLUNTEER","LANGUAGES","PROJECTS","ACHIEVEMENTS","REFERENCES"];
      ...
      // Minimal normalization — only split on exact known headers at start of line
      let norm=text.replace(/\r\n|\r/g,"\n");
      // Insert bullets
      norm=norm.replace(/([^\n])\s*[•·▪]\s*(?=[A-Z])/g,"$1\n• ");
      const rawLines=norm.split("\n").map(l=>l.trim()).filter(Boolean);
      let idx=0;let prevType="";

      for(const line of rawLines){
        const UP=line.toUpperCase().trim();
        const isHdr=KNOWN_HEADERS.some(h=>UP===h);
        const isBul=line.startsWith("•")||line.startsWith("-");
        const dm=line.match(DATE_RE);
        const hasSep=line.includes("|")||line.includes("–")||line.includes("—");
        const isJob=!isHdr&&hasSep&&(dm||line.match(/\d{4}/))&&line.length<160;

        if(idx===0){
          chk(28);block(line,bold,S.nameSize,ML,CW,S.nameSize+5,NAME_C);y-=2;idx++;prevType="name";continue;
        }
        if(idx<=2&&(line.includes("@")||line.match(/\d{3}/))){
          chk(13);block(line,font,8.5,ML,CW,13,DGRAY);y-=2;idx++;prevType="contact";continue;
        }
        if(isHdr){
          y-=prevType==="name"||prevType==="contact"?8:12;chk(22);
          page.drawLine({start:{x:ML,y:y+3},end:{x:PW-MR,y:y+3},thickness:S.ruleThick,color:ACCENT});
          y-=9;block(UP,S.headerBold?bold:font,9.5,ML,CW,14,HDR_C);y-=3;idx++;prevType="section";continue;
        }
        if(isJob){
          y-=prevType==="section"?3:9;chk(20);
          if(dm){
            const ds=dm[0].trim();const tp=line.slice(0,line.search(DATE_RE)).trim();
            const dw=font.widthOfTextAtSize(ds,9);const ry=y;
            block(tp,bold,10,ML,CW-dw-14,16,BLACK);
            page.drawText(ds,{x:PW-MR-dw,y:ry,size:9,font,color:MGRAY});
          }else block(line,bold,10,ML,CW,16,BLACK);
          y-=1;idx++;prevType="job";continue;
        }
        if(isBul){
          const bt=line.replace(/^[•\-]\s*/,"");
          chk(14);page.drawText("•",{x:ML+4,y,size:8.5,font,color:MGRAY});
          block(bt,font,9.5,ML+16,CW-16,14,BLACK);y-=1;idx++;prevType="bullet";continue;
        }
        chk(14);block(line,font,9.5,ML,CW,14,BLACK);idx++;prevType="body";
      }
    }
