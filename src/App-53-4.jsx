...
function ATSModule({country,onScore}){
  ...
  const reset=()=>{setStep(0);setResult(null);setResumeText("");setJobDesc("");setCompany("");setRole("");setActiveTab("optimized");setFileName("");setExtractError("");setApiError("");};
...
CRITICAL: Never fabricate. Weave keywords naturally. Use ${country.name} spelling/terminology.`;
    ...
    if(!res&&lastClaudeError){
      // The API call itself failed (bad/missing key, network, rate limit) —
      ...
      setApiError(lastClaudeError);setLoading(false);setStep(0);return;
    }
    try{
      const clean=res.replace(/```json|```/g,"").trim();
      const s=clean.indexOf("{");const e=clean.lastIndexOf("}")+1;
      ...
      setApiError("The AI response couldn't be read as a resume. This can happen if the model returned something unexpected — please try again.");
    ...
    <div className="panel">
      ...
      <div className="panel-body">
        {apiError&&(
          <div style={{background:"rgba(231,76,60,0.1)",border:"1px solid rgba(231,76,60,0.3)",borderRadius:10,padding:"14px 18px",marginBottom:16,fontSize:13,color:"#E74C3C",lineHeight:1.7}}>
            ⚠️ <strong>The AI request failed:</strong> {apiError}
            <div style={{marginTop:6,fontSize:12,color:"rgba(231,76,60,0.85)"}}>This usually means the Claude API key isn't configured correctly for this deployment, or a network/CORS issue blocked the request. Nothing was faked or fabricated — please fix the API access and try again.</div>
          </div>
        )}
