  ...
  try{
    ...
    for(const line of rawLines){
      ...
      if(isFirst){
        ...
        chk(30);
        // Draw name
        wrapDraw(line,bold,16,ML,maxW,22,NAVY);
        y-=2;
        // Gold rule under name
        page.drawLine({start:{x:ML,y:y+8},end:{x:W-MR,y:y+8},thickness:2,color:GOLD});
        y-=8;
        lineIndex++;
        continue;
      }
      ...
      if(isHeader){
        y-=10;
        chk(22);
        page.drawLine({start:{x:ML,y:y+6},end:{x:W-MR,y:y+6},thickness:0.5,color:GOLD});
        y-=4;
        wrapDraw(line.toUpperCase(),bold,9.5,ML,maxW,15,GOLD);
        y-=2;
        lineIndex++;
        continue;
      }
...
function PDFDownloadButton({text,filename}){
  ...
  const download=async()=>{
    setDownloading(true);
    const success=await generateAndDownloadPDF(text,filename);
    setDownloading(false);
    if(success){setDone(true);setTimeout(()=>setDone(false),3000);}
  };
  return(
    <div style={{marginTop:14,display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
      <button className="btn btn-gold" onClick={download} disabled={downloading||!text}
        style={{fontSize:13,padding:"12px 20px"}}>
        {downloading?"⏳ Generating PDF...":done?"✅ Downloaded!":"📥 Download as PDF"}
      </button>
      <div style={{fontSize:11,color:C.gray}}>Clean professional PDF · Ready to send to employers</div>
    </div>
  );
}
