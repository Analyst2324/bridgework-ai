...
function InterviewGame({country}){
  ...
  const [perFeedback,setPerFeedback]=useState(null); // feedback for current answer
  ...
  const submitAnswer=async()=>{
    ...
    if(!ans.trim())return;
    const updated=[...answers,{q:questions[currentQ],a:ans}];
    setAnswers(updated);
    setTranscript("");setTextInput("");setTimer(120);stopRecording();
    // Show per-question feedback before moving on
    setShowingFeedback(true);
    setPerFeedback(null);
    setPerFeedbackLoading(true);
    const sys=`You are a ${country.name} interview coach giving instant feedback on ONE answer. Be direct, specific, and constructive. Return ONLY a JSON object (no markdown):
{
  "score": number 0-10,
  "scoreLabel": "Weak" | "Fair" | "Good" | "Strong" | "Excellent",
  "usedSTAR": true or false,
  "strength": "One specific thing they did well in 1 sentence",
  "improvement": "One specific thing to improve in 1 sentence",
  "strongerOpener": "Rewrite just the FIRST sentence of their answer to be more impactful and confident"
}`;
    ...
    try{
      ...
      setPerFeedback({score:7,scoreLabel:"Good",usedSTAR:false,strength:"You answered the question directly.",improvement:"Try structuring with STAR: Situation, Task, Action, Result.",strongerOpener:"In my previous role, I led a team that..."});
    }
    setPerFeedbackLoading(false);
    // If last question, after user dismisses feedback → generate final result
    if(currentQ+1>=TOTAL_Q){
      // wait for user to tap Continue, then call generateResult
    }
  };
  ...
  const continueAfterFeedback=()=>{
    ...
    setPerFeedback(null);
    if(currentQ+1>=TOTAL_Q){
      generateResult([...answers]); // answers already includes latest
    } else {
      setCurrentQ(q=>q+1);
    }
  };
  ...
  const generateQuestions=async()=>{
    ...
    const styleNote=country.code==="UK"
      ? "UK interview style: slightly more reserved tone, may include a competency-based question structure, and could touch on the candidate's right to work / visa sponsorship needs."
      : country.code==="CA"
      ? "Canadian interview style: collaborative tone, may probe credential recognition or bilingual ability if relevant, and could touch on PR/work permit status."
      : "US interview style: direct, achievement-focused, may include a question about work authorization status.";
    const sys=`You are a senior hiring manager at ${setup.company} in ${country.name} interviewing for ${setup.role}.
Generate exactly ${TOTAL_Q} interview questions that are specific to the job description, mix behavioral and technical, test whether the candidate's resume claims hold up, and get progressively harder. ${styleNote}
Return ONLY a JSON array of ${TOTAL_Q} question strings — no other text, no markdown fences.`;
    const res=await callClaude([{role:"user",content:`Job Description:\n${setup.jobDesc}\n\nCandidate Resume:\n${setup.resumeText||"Not provided — ask general but role-relevant questions"}`}],sys);
    try{
      const clean=res.replace(/```json|```/g,"").trim();
      const s=clean.indexOf("[");const e=clean.lastIndexOf("]")+1;
  ...
  const fallbackQuestions=()=>["Tell me about yourself and why you're interested in this role.","What's your greatest professional achievement?","Describe a time you led a team through a difficult challenge.","How do you handle conflict with a colleague or manager?","Why do you want to work at "+setup.company+" specifically?","Where do you see yourself in five years, and how does this role fit that path?"];

  const generateResult=async(allAnswers)=>{
    setPhase("result");setLoading(true);
    const sys=`You are a senior hiring panel evaluating a candidate for ${setup.role} at ${setup.company} in ${country.name}.
Evaluate all answers honestly and return ONLY a JSON object (no markdown fences) with this exact shape:
{
...
"missedBuzzwords":[8-12 technical terms or buzzwords relevant to this role AND market (${country.name}) they should weave into answers],
"strongerResponses":[{"q":"question text","original":"brief paraphrase of what they said","improved":"a stronger STAR-method answer using relevant buzzwords"}] for their 2 weakest answers,
"hiringVerdict":"2-3 sentence honest explanation of the decision, written the way a ${country.name} hiring manager would phrase it"
}`;
    const transcriptText=allAnswers.map((a,i)=>`Q${i+1}: ${a.q}\nA: ${a.a}`).join("\n\n");
    const res=await callClaude([{role:"user",content:`Job: ${setup.role} at ${setup.company} (${country.name})\n\nJob Description:\n${setup.jobDesc}\n\nInterview Transcript:\n${transcriptText}`}],sys,1600);
    try{
      const clean=res.replace(/```json|```/g,"").trim();
      const s=clean.indexOf("{");const e=clean.lastIndexOf("}")+1;
      ...
      setResult({decision:"TRY AGAIN",overallScore:62,starScore:55,confidenceScore:60,relevanceScore:65,
        strengths:["Showed genuine enthusiasm for the role","Drew on relevant past experience","Answered every question without dodging"],
        improvements:["Use specific numbers and outcomes more often","Structure answers with the STAR method","Research the company's products and recent news more deeply"],
        missedBuzzwords:["KPI","stakeholder management","cross-functional","scalability","ROI","process optimization","deliverables","agile"],
        strongerResponses:[],
        hiringVerdict:"Solid potential but answers need sharper structure and more concrete, quantified results."});
    }
    setLoading(false);
  };
  ...
  if(phase==="setup")return(
    <div className="panel">
      <div className="panel-header"><span className="panel-icon">🎮</span><div><div className="panel-title">Interview Simulation</div><div className="panel-subtitle">{country.flag} {country.name} style · Voice answers · Hired or Try Again verdict</div></div></div>
      <div className="panel-body">
        <div className="warn-banner">🎯 Paste the real job description. Questions are generated <strong>specifically from it and your {country.docName.toLowerCase()}</strong> — just like a real {country.name} interviewer would.</div>
        <div className="row">
        ...
        <textarea className="textarea-field" style={{minHeight:120}} placeholder="Paste the full job description — responsibilities, requirements, qualifications..." value={setup.jobDesc} onChange={e=>setSetup(p=>({...p,jobDesc:e.target.value}))}/>
        <label className="input-label">Your {country.docName} (optional, recommended)</label>
        <textarea className="textarea-field" placeholder={`Paste your ${country.docName.toLowerCase()} text so questions probe YOUR specific background...`} value={setup.resumeText} onChange={e=>setSetup(p=>({...p,resumeText:e.target.value}))}/>
        <button className="btn btn-gold" style={{marginTop:6}} disabled={!setup.company||!setup.role||!setup.jobDesc||loading} onClick={generateQuestions}>
          🎬 Start Interview
        </button>
      </div>
    </div>
  ...
  if(phase==="briefing")return(
    <div className="panel">
      <div className="panel-header"><span className="panel-icon">📋</span><div><div className="panel-title">Preparing Your Interview</div><div className="panel-subtitle">{setup.role} at {setup.company}</div></div></div>
      <div className="panel-body"><div className="thinking"><div className="dots"><span/><span/><span/></div>Reading the job description and building {TOTAL_Q} targeted questions...</div></div>
    </div>
  );

  if(phase==="result")return(
    <div className="panel">
      <div className="panel-header"><span className="panel-icon">📊</span><div><div className="panel-title">Interview Results</div><div className="panel-subtitle">{setup.role} at {setup.company}</div></div></div>
      <div className="panel-body">
        {loading
          ? <div className="thinking"><div className="dots"><span/><span/><span/></div>Evaluating your performance...</div>
          : result && (
            <>
              ...
              {result.missedBuzzwords?.length>0&&(
                <div className="feedback-section">
                  <div className="feedback-title">⚡ Buzzwords & Technical Terms You Should Have Used</div>
                  <div style={{fontSize:12,color:C.gray,marginBottom:10}}>These signal fluency in the field — weave them naturally into your next attempt.</div>
      ...
      <div className="game-body">
        <div className="interviewer-area">
          ...
          <div className="interviewer-info">
            <div className="interviewer-name">Alex Rivera</div>
            <div className="interviewer-title">Hiring Manager · {setup.company}</div>
            <div className="interviewer-speaking"><div className="speaking-dot"/><span style={{fontSize:11,color:C.verified}}>Asking question {currentQ+1}</span></div>
          </div>
          <div className={`timer-ring${timer<=30?" urgent":""}`}>{fmt(timer)}</div>
        </div>
        <div className="question-box">
          <div className="question-num">Question {currentQ+1} of {TOTAL_Q}</div>
          <div className="question-text">{q}</div>
        </div>
        <div className="voice-area">
          {/* PER-QUESTION FEEDBACK SCREEN */}
          {showingFeedback?(
            <div>
              {perFeedbackLoading?(
                ...
                <div>
                  {/* Score bar */}
                  <div style={{background:"rgba(0,0,0,0.2)",borderRadius:14,padding:"20px 22px",marginBottom:14,display:"flex",alignItems:"center",gap:18}}>
                    <div style={{textAlign:"center",flexShrink:0}}>
                      <div style={{fontFamily:"Syne,sans-serif",fontSize:36,fontWeight:800,color:perFeedback.score>=8?C.verified:perFeedback.score>=5?C.gold:"#E74C3C",lineHeight:1}}>{perFeedback.score}<span style={{fontSize:16,color:C.gray}}>/10</span></div>
                      <div style={{fontSize:11,fontWeight:700,color:C.gray,marginTop:3,letterSpacing:1,textTransform:"uppercase"}}>{perFeedback.scoreLabel}</div>
                    </div>
                    <div style={{flex:1}}>
                  ...
                  {/* Strength */}
                  <div style={{background:"rgba(0,200,150,0.07)",border:"1px solid rgba(0,200,150,0.2)",borderRadius:12,padding:"14px 18px",marginBottom:10,display:"flex",gap:12,alignItems:"flex-start"}}>
                    <span style={{fontSize:18,flexShrink:0}}>✅</span>
                    <div>
                      <div style={{fontSize:10,fontWeight:700,color:C.verified,letterSpacing:1.5,textTransform:"uppercase",marginBottom:4}}>What worked</div>
                      <div style={{fontSize:13,color:C.cream,lineHeight:1.7}}>{perFeedback.strength}</div>
                    </div>
                  </div>

                  {/* Improvement */}
                  <div style={{background:"rgba(232,168,56,0.07)",border:"1px solid rgba(232,168,56,0.2)",borderRadius:12,padding:"14px 18px",marginBottom:10,display:"flex",gap:12,alignItems:"flex-start"}}>
                    <span style={{fontSize:18,flexShrink:0}}>🔧</span>
                    <div>
                      <div style={{fontSize:10,fontWeight:700,color:C.gold,letterSpacing:1.5,textTransform:"uppercase",marginBottom:4}}>One thing to improve</div>
                      <div style={{fontSize:13,color:C.cream,lineHeight:1.7}}>{perFeedback.improvement}</div>
                    </div>
                  </div>

                  {/* Stronger opener */}
                  <div style={{background:"rgba(108,99,255,0.07)",border:"1px solid rgba(108,99,255,0.2)",borderRadius:12,padding:"14px 18px",marginBottom:18,display:"flex",gap:12,alignItems:"flex-start"}}>
                    <span style={{fontSize:18,flexShrink:0}}>💬</span>
                    <div>
                      <div style={{fontSize:10,fontWeight:700,color:"#9B93FF",letterSpacing:1.5,textTransform:"uppercase",marginBottom:4}}>Stronger opening line</div>
                      <div style={{fontSize:13,color:C.cream,lineHeight:1.7,fontStyle:"italic"}}>"{perFeedback.strongerOpener}"</div>
                    </div>
                  </div>
                  ...
                  <button className="btn btn-gold" onClick={continueAfterFeedback} style={{width:"100%",justifyContent:"center",fontSize:14,padding:"14px"}}>
                    {currentQ+1>=TOTAL_Q?"See Final Results 📊":"Next Question →"}
                  </button>
                </div>
              )}
            </div>
            ...
            !textMode?(
              <div className="voice-status">
                ...
                <div className="voice-hint">{isRecording?"Recording... tap to stop":"Tap the mic to start your answer"}</div>
...
function VerifyModule({country}){
  ...
  const [form,setForm]=useState({title:"",company:"",startDate:"",endDate:"",refEmail:""});
  ...
  const sendReq=async(exp)=>{
    if(!isOfficial(exp.refEmail))return;
    setSending(exp.id);
  ...
  if(view==="add")return(
    <div className="panel">
      ...
      <div className="panel-body">
        ...
        <input className={`input-field${form.refEmail&&!isOfficial(form.refEmail)?" error":""}`} placeholder="reference@company.com" value={form.refEmail} onChange={e=>setForm(p=>({...p,refEmail:e.target.value}))}/>
        {form.refEmail&&<div className={`email-status ${isOfficial(form.refEmail)?"email-ok":"email-bad"}`}>{isOfficial(form.refEmail)?"✓ Official email accepted":"✗ Must be an official company email"}</div>}
    ...
    <div>
      <div className="verify-score">
        ...
        <div className="score-info">
          ...
          <p>Share your public profile. Employers verify independently — no login needed.</p>
          <div className="public-link">
            <span className="public-link-url">🔗 bridgework.ai/verify/ameen-ajibola</span>
          ...
          {expanded===exp.id&&(
            <div className="ref-form-inner">
              ...
              <div style={{display:"flex",gap:9}}>
                <input className={`input-field flex1${exp.refEmail&&!isOfficial(exp.refEmail)?" error":""}`} style={{marginBottom:0}} placeholder="reference@company.com" value={exp.refEmail||""} onChange={e=>setExps(p=>p.map(x=>x.id===exp.id?{...x,refEmail:e.target.value}:x))}/>
                <button className="btn btn-green btn-sm" disabled={!isOfficial(exp.refEmail||"")||sending===exp.id} onClick={()=>sendReq(exp)}>{sending===exp.id?"Sending...":"Send"}</button>
              </div>
              {exp.refEmail&&<div className={`email-status ${isOfficial(exp.refEmail)?"email-ok":"email-bad"}`}>{isOfficial(exp.refEmail)?"✓ Good to go":"✗ Official company email required"}</div>}
            </div>
          )}
        </div>
      ))}
