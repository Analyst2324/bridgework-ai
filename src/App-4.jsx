...
function QuestionBankModule({country}){
  ...
  const loadQuestions=async()=>{
    setLoading(true);
    try{
      const keys=await window.storage.list("iq:",true);
      const items=await Promise.all(
        (keys.keys||[]).map(async k=>{
          try{const r=await window.storage.get(k,true);return r?JSON.parse(r.value):null;}
          catch{return null;}
...
Approve if: it's a real interview question someone could be asked. Reject if: spam, gibberish, offensive, not a question, or completely unrelated to job interviews.`;
    ...
    try{
      ...
      try{const s=await window.storage.get("iq:summary",true);if(s)summary=JSON.parse(s.value);}catch{}
      summary.total+=1;
      if(status==="approved")summary.approved+=1;
      else summary.flagged+=1;
      summary.byCountry[form.country]=(summary.byCountry[form.country]||0)+1;
      await window.storage.set("iq:summary",JSON.stringify(summary),true);
    }catch(e){console.error(e);}
...
Be specific to the role and company context. Keep it practical and direct.`;
    ...
    try{
      ...
      await window.storage.set(q.id,JSON.stringify(updated),true);
    }catch{}
  };
...
function ATSModule({country}){
  ...
  const [result,setResult]=useState(null);
  const [loading,setLoading]=useState(false);
  const [activeTab,setActiveTab]=useState("optimized"); // optimized|keywords|tips
...
{
  "beforeScore": number 0-100 (how well current resume matches JD for ATS),
  "afterScore": number 0-100 (projected score after optimization),
  "missingKeywords": ["keyword1","keyword2",...] (important JD keywords completely absent from resume — max 15),
  "presentKeywords": ["keyword1","keyword2",...] (JD keywords already in resume — max 10),
  "weakSections": ["section name: specific issue"] (2-3 resume sections that need strengthening for this role),
  "optimizedResume": "full rewritten resume with missing keywords woven in naturally, ATS-friendly formatting, role-specific language from the JD. Keep all true experience intact — only add/rephrase, never fabricate.",
  "keyChanges": ["change 1","change 2","change 3","change 4","change 5"] (exactly 5 specific changes made),
  "atsWarnings": ["warning 1","warning 2"] (1-3 ATS red flags found: tables, graphics mentions, unusual formatting etc),
  "recruiterTip": "One insider tip specific to this company/role combination"
}

CRITICAL RULES:
- Never fabricate experience or skills the candidate doesn't have
- Weave in keywords naturally — not as a keyword dump
- Match the seniority and tone of the job description
- Use ${country.name}-specific terminology and spelling`;
    ...
    try{
      ...
      setResult({
        beforeScore:42,afterScore:78,
        missingKeywords:["stakeholder management","cross-functional","KPI","P&L","agile","deliverables","ROI","scalability"],
        presentKeywords:["operations","management","team","analysis"],
        weakSections:["Summary: too generic, doesn't mirror JD language","Experience: missing quantified outcomes","Skills: not aligned to JD requirements"],
        optimizedResume:resumeText,
        keyChanges:["Added 8 missing ATS keywords naturally","Rewrote summary to mirror JD language","Quantified 3 achievement bullets","Aligned skills section to JD requirements","Adjusted job titles to match market standard"],
        atsWarnings:["Avoid tables in resume — many ATS systems can't parse them"],
        recruiterTip:"This role prioritises stakeholder management — lead every bullet with that angle."
      });
      setStep(2);
    }
    setLoading(false);
  ...
  const reset=()=>{setStep(0);setResult(null);setResumeText("");setJobDesc("");setCompany("");setRole("");setActiveTab("optimized");};
  ...
  if(step===0)return(
    <div className="panel">
      <div className="panel-header">
        <span className="panel-icon">🎯</span>
        <div>
          <div className="panel-title">ATS Resume Optimizer</div>
          <div className="panel-subtitle">Paste a job description — we'll rewrite your resume to pass their ATS filter</div>
        </div>
      </div>
      <div className="panel-body">
        <div style={{background:"rgba(231,76,60,0.07)",border:"1px solid rgba(231,76,60,0.2)",borderRadius:10,padding:"14px 18px",marginBottom:20,fontSize:13,color:C.cream,lineHeight:1.75}}>
          ⚠️ <strong style={{color:"#E74C3C"}}>75% of resumes never reach a human.</strong> ATS systems filter them out first — not because you're unqualified, but because your resume doesn't use the exact language the system is scanning for. This fixes that.
        </div>

        <div className="row">
          <div className="flex1">
            <label className="input-label">Company Name</label>
            <input className="input-field" placeholder="e.g. Barclays, Amazon, KPMG" value={company} onChange={e=>setCompany(e.target.value)}/>
          </div>
          <div className="flex1">
            <label className="input-label">Job Title</label>
            <input className="input-field" placeholder="e.g. Senior Operations Manager" value={role} onChange={e=>setRole(e.target.value)}/>
          </div>
        </div>

        <label className="input-label">Job Description *</label>
        <textarea className="textarea-field" style={{minHeight:140}}
          placeholder="Paste the full job description here — responsibilities, requirements, qualifications, everything..."
          value={jobDesc} onChange={e=>setJobDesc(e.target.value)}/>

        <label className="input-label">Your Current Resume * (paste as text)</label>
        <textarea className="textarea-field" style={{minHeight:160}}
          placeholder="Paste your full resume text here. Open your CV in Word/Google Docs → Select All → Copy → Paste here."
          value={resumeText} onChange={e=>setResumeText(e.target.value)}/>

        <div style={{background:"rgba(0,200,150,0.06)",border:"1px solid rgba(0,200,150,0.15)",borderRadius:10,padding:"12px 16px",marginBottom:16,fontSize:12,color:C.cream,lineHeight:1.7}}>
          ✅ <strong style={{color:C.verified}}>Your experience is never changed.</strong> We only add missing keywords and rephrase existing bullets to match the JD language. Everything stays true to your actual background.
        </div>
...
const NAV=[
  ...
  {id:"ats",icon:"🎯",label:"ATS Optimizer",badge:"NEW"},
      ...
      <div className="app">
        ...
        <main className="main">
          ...
          <div className="content">
            ...
            {active==="ats"&&<ATSModule country={country}/>}
