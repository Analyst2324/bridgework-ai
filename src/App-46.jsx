...
function PDFDownloadButton({text,filename}){
  ...
  const downloadPDF=async()=>{
    ...
    try{
      ...
      if(!window.docx){
        await new Promise((res,rej)=>{
          const timeout=setTimeout(()=>rej(new Error("docx load timeout")),20000);
          const s=document.createElement("script");
          s.src="https://unpkg.com/docx@8.2.3/build/index.js";
          s.onload=()=>{clearTimeout(timeout);res();};
          s.onerror=()=>{
            const s2=document.createElement("script");
            s2.src="https://cdn.jsdelivr.net/npm/docx@8.2.3/build/index.js";
            s2.onload=()=>{clearTimeout(timeout);res();};
            s2.onerror=()=>{clearTimeout(timeout);rej(new Error("docx library failed to load"));};
            document.head.appendChild(s2);
          };
          document.head.appendChild(s);
        });
      ...
      const d=window.docx;
      if(!d||!d.Document||!d.Packer||!d.Paragraph||!d.TextRun){
        throw new Error("docx library incomplete: "+Object.keys(d||{}).join(","));
      }

      const {Document,Packer,Paragraph,TextRun,BorderStyle}=d;
      // twip = 1/1440 inch
      const twip=(n)=>Math.round(n*1440);
      const NAVY="0D1B2A",GOLD="C8881A",GRAY="666666",BLACK="111111";

      const san=(s)=>(s||"").replace(/[━─═]/g,"-").replace(/[""]/g,'"').replace(/['']/g,"'").replace(/[…]/g,"...").replace(/[^ -•–—\-|]/g,"");

      const HDRS=["PROFESSIONAL SUMMARY","CORE COMPETENCIES","PROFESSIONAL EXPERIENCE","WORK EXPERIENCE","EARLY CAREER — EMERGING MARKETS EXPERIENCE","EARLY CAREER","EDUCATION & CERTIFICATIONS","EDUCATION","CERTIFICATIONS","SKILLS","SYSTEMS & TOOLS","KEY COMPETENCIES","ACHIEVEMENTS","REFERENCES","ADDITIONAL","TITLE TRANSLATIONS","KEY UPGRADES"];
      const DTRE=/((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\w*\.?\s+\d{4}\s*[-–—]+\s*(?:(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\w*\.?\s+\d{4}|Present))\s*$/i;

      const lines=text.split("\n").map(l=>san(l.trim())).filter(Boolean);
      const ch=[];
      let idx=0;

      const mkPara=(runs,opts={})=>new Paragraph({children:runs,...opts});
      const mkRun=(t,opts={})=>new TextRun({text:String(t),...opts,font:"Calibri"});

      for(const line of lines){
        if(!line)continue;
        const UP=line.toUpperCase().trim();
        const isHdr=HDRS.some(h=>UP===h||UP.startsWith(h));
        const isBul=line.startsWith("•")||(/^- /.test(line));
        const dm=line.match(DTRE);
        const hasSep=line.includes("|")||line.includes("-")||line.includes("—")||line.includes("–");
        const isJob=!isHdr&&hasSep&&!!line.match(/\d{4}/)&&line.length<220;

        // Name
        if(idx===0){
          ch.push(mkPara([mkRun(line,{bold:true,size:34,color:NAVY})],{
            spacing:{after:60},
            border:{bottom:{color:"C8881A",space:4,style:BorderStyle?BorderStyle.SINGLE:"single",size:8}},
          }));
          idx++;continue;
        }
        ...
        if(idx<=3&&(line.includes("@")||/\d{3}/.test(line)||line.toLowerCase().includes("linkedin"))){
          ch.push(mkPara([mkRun(line,{size:18,color:GRAY})],{spacing:{after:80}}));
          idx++;continue;
        }

        // Section header
        if(isHdr){
          ch.push(mkPara([mkRun(UP,{bold:true,size:20,color:GOLD,allCaps:true})],{
            spacing:{before:200,after:60},
            border:{bottom:{color:"C8881A",space:1,style:BorderStyle?BorderStyle.SINGLE:"single",size:4}},
          }));
          idx++;continue;
        }
        ...
        // Job title line
        if(isJob){
          const ds=dm?dm[0].trim():"";
          const tp=ds?line.slice(0,line.search(DTRE)).trim():line;
          const runs=[mkRun(tp,{bold:true,size:22,color:NAVY})];
          if(ds)runs.push(mkRun("    "+ds,{size:18,color:GRAY}));
          ch.push(mkPara(runs,{spacing:{before:120,after:40}}));
          idx++;continue;
        }
        ...
        if(isBul){
          const bt=line.replace(/^[•-]\s*/,"");
          ch.push(mkPara([mkRun(bt,{size:20,color:BLACK})],{
        ...
        // Body
        ch.push(mkPara([mkRun(line,{size:20,color:BLACK})],{spacing:{after:40}}));
        idx++;
      }
      ...
      const doc=new Document({
        sections:[{
          properties:{page:{margin:{
            top:twip(0.75),bottom:twip(0.75),
            left:twip(0.75),right:twip(0.75),
          }}},
          children:ch,
        }],
      });
