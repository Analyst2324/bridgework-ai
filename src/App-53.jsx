  ...
  const downloadPDF=async()=>{
    ...
    try{
      ...
      if(isStructured){
        ...
        const d=data;
        // Name
        chk(30);wrap(cl(d.name),B,S.nameSize,ML,CW,S.nameSize+5,NC);
        rule(S.ruleThick*1.5,AC);y-=8;
        // Contact
        if(d.contact){chk(14);wrap(cl(d.contact),F,8.5,ML,CW,13,GR);y-=4;}
        // Sections
        d.sections?.forEach(sec=>{
          y-=10;chk(22);
          rule(S.ruleThick,AC);y-=9;
          wrap(cl(sec.heading),S.headerBold?B:F,9.5,ML,CW,14,HC);y-=3;
          sec.items?.forEach(item=>{
            if(item.type==="job"){
              y-=6;chk(20);
              const ds=cl(item.date||"");
              const dw=ds?F.widthOfTextAtSize(ds,9):0;
              const tp=cl(item.title||"");
              const ry=y;
              wrap(tp,B,10,ML,dw?CW-dw-12:CW,16,BK);
              if(ds)pg.drawText(ds,{x:PW-MR-dw,y:ry,size:9,font:F,color:GR});
              if(y===ry)y-=16;
              item.bullets?.forEach(b=>{
                chk(14);
                pg.drawText("\u2022",{x:ML+4,y,size:9,font:F,color:GR});
                wrap(cl(b),F,9.5,ML+14,CW-14,14,BK);
                y-=1;
              });
            }else if(item.type==="edu"){
              chk(16);
              ...
              const instW=inst?F.widthOfTextAtSize(inst+date,9):0;
              const ry=y;
              wrap(cl(item.degree||""),B,9.5,ML,inst?CW-instW-10:CW,15,BK);
              if(inst)pg.drawText(inst+date,{x:PW-MR-instW,y:ry,size:9,font:F,color:GR});
              if(y===ry)y-=15;
            }else if(item.type==="bullet"){
              chk(14);
              pg.drawText("\u2022",{x:ML+4,y,size:9,font:F,color:GR});
              wrap(cl(item.text||""),F,9.5,ML+14,CW-14,14,BK);
              y-=1;
            }else{
              chk(14);wrap(cl(item.text||""),F,9.5,ML,CW,14,BK);
            }
          });
        });
      }else{
