...
async function generateAndDownloadPDF(text, filename="CrossBorder-CV.pdf", templateId="modern"){
  try{
    ...
    // Word-wrap a text block, returns nothing, advances y
    ...
    // ── EXACT SECTION HEADERS (only match whole line) ──
    const HEADERS=[
      "PROFESSIONAL SUMMARY","CORE COMPETENCIES","SYSTEMS & TOOLS","TECHNICAL SKILLS",
      "PROFESSIONAL EXPERIENCE","WORK EXPERIENCE","EARLY CAREER — EMERGING MARKETS EXPERIENCE",
      "EARLY CAREER — EMERGING MARKETS","EARLY CAREER",
      "EDUCATION & CERTIFICATIONS","EDUCATION AND CERTIFICATIONS","EDUCATION","CERTIFICATIONS",
      "ADDITIONAL INFORMATION","AWARDS","VOLUNTEER","LANGUAGES","PROJECTS","ACHIEVEMENTS","REFERENCES"
    ];

    // ── SMART LINE CLASSIFIER ──
    // The key insight: we trust the AI output structure.
    // Section headers are on their OWN line in ALL CAPS.
    // Job titles have a date pattern at the end.
    // Bullets start with •
    ...
    const DEGREE_RE=/^(B\.?Tech|B\.?Sc|M\.?Sc|MBA|Ph\.?D|B\.?A\b|M\.?A\b|Bachelor|Master|Doctor|Certificate|Diploma|Evolve|CFA|Cyber)/i;

    // Normalize — ONLY add newlines, never remove content
    // Step 1: normalize line endings
    let norm=text.replace(/\r\n|\r/g,"\n");

    // Step 2: insert newline before EXACT section headers that appear mid-text
    // Only match if the header is preceded by a non-newline char (i.e. inline)
    for(const h of HEADERS){
      const esc=h.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");
      // Add newline before header only if it's preceded by non-whitespace (runon text)
    ...
    // Step 4: split "SectionHeader JobTitle" — header immediately followed by Title Case
    for(const h of HEADERS){
      const esc=h.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");
      norm=norm.replace(new RegExp(`(${esc})[ \t]+([A-Z][a-z])`, "g"),"$1\n$2");
      norm=norm.replace(new RegExp(`(${esc})[ \t]+([A-Z][A-Z])`, "g"),"$1\n$2");
    }

    // Step 5: split "Bullet text•Next bullet" inline bullets
    norm=norm.replace(/([a-z,])\s*•\s*([A-Z])/g,"$1\n• $2");

    // Step 6: break education entries — "degree  institution" (3+ spaces = tab-like separator)
    norm=norm.replace(/([A-Za-z\(\),]+)\s{3,}([A-Z][a-z])/g,"$1\t$2");
    ...
    for(const line of lines){
      ...
      // Classify
      const isHeader=HEADERS.some(h=>UP===h||UP===h.toUpperCase());
      const isBullet=line.startsWith("•")||line.startsWith("-");
      const dateMatch=line.match(DATE_RE);
      const hasSep=line.includes("|")||line.includes("–")||line.includes("—");
      const isJob=!isHeader&&hasSep&&(dateMatch||line.match(/\d{4}/))&&line.length<160;
      const isDegree=!isHeader&&!isJob&&DEGREE_RE.test(line);
      const hasTab=line.includes("\t");

      // ── NAME (first line) ──
      if(lineIdx===0){
        chk(28);
        block(line,bold,S.nameSize,ML,CW,S.nameSize+4,NAME_C);
        y-=1;
        lineIdx++;prevType="name";continue;
      }

      // ── CONTACT (lines 1-2 with @ or phone) ──
      if(lineIdx<=2&&(line.includes("@")||line.match(/\d{3}[\s\-]\d{3}/)||line.toLowerCase().includes("linkedin")||line.includes("•"))){
        chk(13);
        // render contact bullets inline
        const parts=line.split(/\s*•\s*/).filter(Boolean);
        if(parts.length>1){
          for(const p of parts){
            chk(13);
            page.drawText("•",{x:ML,y,size:8,font,color:MGRAY});
            block(p.trim(),font,8.5,ML+10,CW-10,13,DGRAY);
          }
        }else{
          block(line,font,8.5,ML,CW,13,DGRAY);
        }
        lineIdx++;prevType="contact";continue;
      }
      ...
      if(isHeader){
        ...
        y-=8;
        block(UP,S.headerBold?bold:font,9.5,ML,CW,14,HDR_C);
        y-=3;
        lineIdx++;prevType="header";continue;
      }

      // ── JOB TITLE LINE ──
      if(isJob){
        y-=prevType==="header"?4:9;
        chk(22);
        if(dateMatch){
          const dateStr=dateMatch[0].trim();
          const idx=line.search(DATE_RE);
          const titlePart=line.slice(0,idx).trim().replace(/\s+$/,"");
          const dateW=font.widthOfTextAtSize(dateStr,9);
          const rowY=y;
          // Draw title bold left
          const titleWords=titlePart.split(/\s+/);
          let tline="";
          const titleMax=CW-dateW-14;
          for(const w of titleWords){
            const t=tline?`${tline} ${w}`:w;
            if(bold.widthOfTextAtSize(t,10)>titleMax&&tline){
              chk(15);page.drawText(tline,{x:ML,y,size:10,font:bold,color:BLACK});y-=15;tline=w;
            }else tline=t;
          }
          if(tline){chk(15);page.drawText(tline,{x:ML,y,size:10,font:bold,color:BLACK});}
          // Date right-aligned on same row as first title line
          page.drawText(dateStr,{x:PW-MR-dateW,y:rowY,size:9,font,color:MGRAY});
          y-=16;
        }else{
          block(line,bold,10,ML,CW,16,BLACK);
        }
        lineIdx++;prevType="job";continue;
      }
      ...
      if(isBullet){
        ...
        y-=1;
        lineIdx++;prevType="bullet";continue;
      }

      // ── EDUCATION ENTRY (degree \t institution) ──
      if(hasTab){
        const parts=line.split("\t");
        const deg=parts[0].trim();
        const inst=parts[1]?.trim()||"";
        chk(14);
        const rowY=y;
        block(deg,bold,9.5,ML,CW*0.62,14,BLACK);
        if(inst){
          const instW=italic.widthOfTextAtSize(inst,9);
          const instX=Math.max(ML+CW*0.62,PW-MR-instW);
          page.drawText(inst,{x:instX,y:rowY,size:9,font:italic,color:MGRAY});
        }
        lineIdx++;prevType="edu";continue;
      }

      // ── DEGREE LINE (bold, no institution) ──
      if(isDegree){
        chk(14);
        block(line,bold,9.5,ML,CW,14,BLACK);
        lineIdx++;prevType="edu";continue;
      }
