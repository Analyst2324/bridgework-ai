  ...
  const sendReq=async(exp)=>{
    ...
    try{
      ...
      await window.emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, {
        ...
        company:        exp.company,
        dates:          exp.dates,
        verify_link:    `https://getcrossborder.com/verify?id=${exp.id}&ref=${btoa(exp.refEmail)}`,
        reply_to:       "noreply@getcrossborder.com",
        name:           candidateName || "A CrossBorder AI candidate",
        message:        `${candidateName||"A candidate"} is requesting verification of their role as ${exp.title} at ${exp.company} (${exp.dates}). Please reply to confirm this is accurate.`,
      });

      // Save to localStorage for tracking
      storage.set(`verify:${exp.id}`, JSON.stringify({
        expId: exp.id,
        title: exp.title,
        company: exp.company,
      ...
      setSendError("Could not send email. Please check your EmailJS setup or try again.");
    }
    setSending(null);
  };
          ...
          {expanded===exp.id&&(
            <div className="ref-form-inner">
              ...
              <div style={{display:"flex",gap:9}}>
                <input className={`input-field flex1${isOfficial(exp.refEmail||"")===false?" error":""}`} style={{marginBottom:0}} placeholder="reference@company.com" value={exp.refEmail||""} onChange={e=>setExps(p=>p.map(x=>x.id===exp.id?{...x,refEmail:e.target.value}:x))}/>
                <button className="btn btn-green btn-sm" disabled={isOfficial(exp.refEmail||"")!==true||sending===exp.id} onClick={()=>sendReq(exp)}>{sending===exp.id?"Sending...":"Send"}</button>
              </div>
              {isOfficial(exp.refEmail||"")===true&&<div className="email-status email-ok">✓ Good to go</div>}
