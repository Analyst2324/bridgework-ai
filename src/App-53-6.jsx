...
let lastClaudeError=null; // last API failure reason, so callers that want to
                           // surface real errors (instead of silently treating
                           // "" as a valid empty response) can check it right
                           // after their own await callClaude(...) call.
async function callClaude(messages,system,maxTokens=1000){
  ...
  try{
    ...
    if(!r.ok){
      lastClaudeError=d?.error?.message||`Claude API error (HTTP ${r.status})`;
      console.error("Claude API error:",d);
      return "";
    }
    return d.content?.map(b=>b.text||"").join("")||"";
  }catch(e){
    lastClaudeError=e?.message||"Network error contacting Claude API";
    console.error("Claude fetch error:",e);
    return "";
  }
}
// The model sometimes returns text with runs of several blank lines between
// items (especially list-like content such as skills or bullets). Rendered
// with white-space:pre-wrap that becomes a huge, unreadable vertical gap on
// screen. Collapse any run of blank lines down to a single line break before
// displaying raw AI text anywhere in the app.
const tidyAIText=(t)=>String(t||"")
  .replace(/\r\n/g,"\n")
  .replace(/[ \t]+$/gm,"")
  .replace(/\n{2,}/g,"\n")
  .trim();
...
Rules:
1. TITLE TRANSLATION: Elevate titles to ${country.name} equivalents (${morphExamples})
2. QUANTIFY: Add numbers and impact wherever possible
...
Return ONLY a JSON object (no markdown, no extra text):
{
  ...
  "titleTranslations": ["Old Title → New Title", ...],
  "sections": [
    {
      "heading": "SECTION NAME",
      "items": [
        {"type": "job", "title": "Job Title | Company – Location", "date": "Jan 2022 – Present", "bullets": ["bullet 1", "bullet 2"]},
        {"type": "bullet", "text": "bullet text"},
        {"type": "body", "text": "paragraph text"},
        {"type": "edu", "degree": "Degree Name", "institution": "University Name", "date": "2020"}
      ]
    }
  ],
  "keyUpgrades": ["upgrade 1", "upgrade 2", "upgrade 3"]
}
Section types: job entries use type "job" with title/date/bullets. Skills/competencies use type "body". Education uses type "edu".`;
    setApiError("");
    ...
    if(!res&&lastClaudeError){
      setApiError(lastClaudeError);setLoading(false);setStep(1);return;
    }
    // Parse JSON result
    let parsed=null;
    try{
      ...
      parsed=JSON.parse(clean.slice(s,e));
    }catch(err){parsed=null;}
    setResult(parsed||res);setLoading(false);
    onComplete&&onComplete();
    const textForConfidence=parsed?`Name: ${parsed.name}\n${parsed.sections?.map(s=>`${s.heading}:\n${s.items?.map(i=>i.type==="job"?`${i.title} ${i.date}\n${i.bullets?.map(b=>`• ${b}`).join("\n")}`:`${i.text||i.degree}`).join("\n")}`).join("\n\n")}`:(res||"");
    setTimeout(()=>startConfidence(textForConfidence),400);
  };
  ...
  return(
    <div className="panel">
      ...
      <div className="panel-body">
        ...
        {apiError&&(
          <div style={{background:"rgba(231,76,60,0.1)",border:"1px solid rgba(231,76,60,0.3)",borderRadius:10,padding:"14px 18px",marginBottom:16,fontSize:13,color:"#E74C3C",lineHeight:1.7}}>
            ⚠️ <strong>The AI request failed:</strong> {apiError}
            <div style={{marginTop:6,fontSize:12,color:"rgba(231,76,60,0.85)"}}>This usually means the Claude API key isn't configured correctly for this deployment, or a network/CORS issue blocked the request.</div>
          </div>
        )}
              ...
              : <>
                  ...
                  {result&&typeof result==="object"?(
                    <div className="ai-response">
                      <div className="ai-label">{country.flag} Transformed {country.docName}</div>
                      <div style={{fontFamily:"'Inter',sans-serif",fontSize:13,lineHeight:1.8,color:C.cream}}>
                        {result.titleTranslations?.length>0&&(
                          <div style={{marginBottom:12,padding:"8px 12px",background:"rgba(232,168,56,0.07)",borderRadius:8,borderLeft:`3px solid ${C.gold}`}}>
                            <div style={{fontSize:10,fontWeight:700,color:C.gold,marginBottom:6,letterSpacing:1,textTransform:"uppercase"}}>Title Translations</div>
                            {result.titleTranslations.map((t,i)=><div key={i} style={{fontSize:12,color:C.cream}}>✦ {t}</div>)}
                          </div>
                        )}
                        <div style={{fontWeight:700,fontSize:15,color:C.white,marginBottom:2}}>{result.name}</div>
                        <div style={{fontSize:11,color:C.gray,marginBottom:12}}>{result.contact}</div>
                        {result.sections?.map((sec,si)=>(
                          <div key={si} style={{marginBottom:14}}>
                            <div style={{fontSize:10,fontWeight:700,color:C.gold,letterSpacing:1.5,textTransform:"uppercase",borderBottom:`1px solid rgba(232,168,56,0.2)`,paddingBottom:3,marginBottom:8}}>{sec.heading}</div>
                            {sec.items?.map((item,ii)=>(
                              <div key={ii}>
                                {item.type==="job"&&(
                                  <div style={{marginBottom:6}}>
                                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                                      <div style={{fontWeight:700,fontSize:12,color:C.white,flex:1}}>{item.title}</div>
                                      <div style={{fontSize:11,color:C.gray,marginLeft:8,whiteSpace:"nowrap"}}>{item.date}</div>
                                    </div>
                                    {item.bullets?.map((b,bi)=><div key={bi} style={{fontSize:12,color:C.cream,paddingLeft:12,marginTop:2}}>• {b}</div>)}
                                  </div>
                                )}
                                {item.type==="edu"&&(
                                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                                    <div style={{fontWeight:600,fontSize:12,color:C.white}}>{item.degree}</div>
                                    <div style={{fontSize:11,color:C.gray}}>{item.institution}{item.date?` · ${item.date}`:""}</div>
                                  </div>
                                )}
                                {item.type==="body"&&<div style={{fontSize:12,color:C.cream,marginBottom:4}}>{item.text}</div>}
                                {item.type==="bullet"&&<div style={{fontSize:12,color:C.cream,paddingLeft:12,marginBottom:2}}>• {item.text}</div>}
                              </div>
                            ))}
                          </div>
                        ))}
                        {result.keyUpgrades?.length>0&&(
                          <div style={{marginTop:8,padding:"8px 12px",background:"rgba(0,200,150,0.06)",borderRadius:8,borderLeft:`3px solid ${C.verified}`}}>
                            <div style={{fontSize:10,fontWeight:700,color:C.verified,marginBottom:6,letterSpacing:1,textTransform:"uppercase"}}>Key Upgrades Made</div>
                            {result.keyUpgrades.map((u,i)=><div key={i} style={{fontSize:12,color:C.cream}}>✓ {u}</div>)}
                          </div>
                        )}
                      </div>
                    </div>
                  ):(
                    <div className="ai-response"><div className="ai-label">{country.flag} Transformed {country.docName}</div><div className="ai-content">{tidyAIText(result)}</div></div>
                  )}
                  <PDFDownloadButton data={result} filename={`CrossBorder-${country.docName}-Transformed.pdf`}/>
                </>
            }
          </div>
        )}
...
// ───────────────────────────────────────────────────────────────────────────
async function generateAndDownloadPDF(text, filename="CrossBorder-CV.pdf"){
  try{
    // Load pdf-lib via script tag (dynamic import fails in browser)
    await new Promise((resolve,reject)=>{
    ...
    const checkY=(needed=lineHeight*2)=>{
    ...
    const drawLine=()=>{
      checkY(8);
      page.drawLine({start:{x:margin,y:y+4},end:{x:pageWidth-margin,y:y+4},thickness:0.5,color:rgb(0.7,0.7,0.7)});
      y-=8;
    };
    ...
    for(const line of lines){
      ...
      if(trimmed===trimmed.toUpperCase()&&trimmed.length>3&&!/^\d/.test(trimmed)){
        y-=6;
        drawLine();
        drawText(trimmed,{bold:true,size:subheadSize,color:rgb(0.1,0.3,0.6)});
        y-=2;
      }
      // Bullet points
        ...
        checkY(lineHeight);
        page.drawText("•",{x:margin+8,y,size:fontSize,font,color:rgb(0.3,0.3,0.3)});
        ...
        for(const word of words){
          ...
          if(w>(maxWidth-24)&&bline){
            checkY(lineHeight);
            page.drawText(bline,{x:margin+20,y,size:fontSize,font,color:rgb(0.15,0.15,0.15)});
            y-=lineHeight;
            bline=word;firstLine=false;
          } else {bline=test;}
        }
        if(bline){
          checkY(lineHeight);
      ...
      else if(lines.indexOf(line)===lines.findIndex(l=>l.trim())){
        drawText(trimmed,{bold:true,size:headingSize,color:rgb(0.05,0.15,0.35)});
        y-=2;
      }
      // Regular text
      else{
        drawText(trimmed,{bold:trimmed.endsWith(":")||trimmed.includes("|")});
      }
    }
    ...
    pages.forEach((p,i)=>{
      p.drawText(`Generated by CrossBorder AI • getcrossborder.com`,{
...
const TEMPLATES=[
  {id:"classic",name:"Classic",icon:"📄",desc:"Clean, ATS-safe",styles:{nameSize:16,nameColor:[0.05,0.1,0.2],accentColor:[0.2,0.2,0.2],headerColor:[0.1,0.1,0.1],ruleThick:0.5,headerBold:true}},
  {id:"modern",name:"Modern",icon:"⚡",desc:"Gold accents",styles:{nameSize:18,nameColor:[0.05,0.11,0.16],accentColor:[0.91,0.66,0.22],headerColor:[0.91,0.66,0.22],ruleThick:1.5,headerBold:true}},
  {id:"executive",name:"Executive",icon:"👔",desc:"C-Suite ready",styles:{nameSize:20,nameColor:[0,0,0],accentColor:[0.15,0.25,0.45],headerColor:[0.15,0.25,0.45],ruleThick:2,headerBold:true}},
  {id:"minimal",name:"Minimal",icon:"✨",desc:"Elegant",styles:{nameSize:17,nameColor:[0.1,0.1,0.1],accentColor:[0.5,0.5,0.5],headerColor:[0.4,0.4,0.4],ruleThick:0.5,headerBold:false}},
];

// Section headers the AI uses
const RESUME_SECTIONS=["PROFESSIONAL SUMMARY","SUMMARY","CORE COMPETENCIES","KEY COMPETENCIES","SKILLS","TECHNICAL SKILLS","SYSTEMS & TOOLS","PROFESSIONAL EXPERIENCE","WORK EXPERIENCE","EXPERIENCE","EMPLOYMENT","EARLY CAREER","EDUCATION & CERTIFICATIONS","EDUCATION AND CERTIFICATIONS","EDUCATION","CERTIFICATIONS","ACHIEVEMENTS","AWARDS","REFERENCES","ADDITIONAL","LANGUAGES","VOLUNTEER","PROJECTS","KEY UPGRADES","TITLE TRANSLATIONS","TITLE TRANSLATIONS MADE"];

// Match dates at end of line: "02/2026 to Current", "Jan 2024 - Present", "2022 - 2024"
const JOB_DATE=/(\d{1,2}\/\d{4}|(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\w*\.?\s+\d{4}|\d{4})\s*(to|-|–|—)\s*(\d{1,2}\/\d{4}|(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\w*\.?\s+\d{4}|\d{4}|Current|Present)\s*$/i;

function PDFDownloadButton({data,text,filename}){
  ...
  // Resolve: data may be structured JSON or raw text (fallback)
  const isStructured=data&&typeof data==="object"&&data.sections;
  const rawText=isStructured?null:(typeof data==="string"?data:text||"");

  // Convert structured JSON to flat text for clipboard
  const toPlainText=(d)=>{
    if(!d)return"";
    const lines=[];
    lines.push(d.name||"");
    ...
    lines.push("");
    if(d.titleTranslations?.length){
      lines.push("TITLE TRANSLATIONS");
      d.titleTranslations.forEach(t=>lines.push(t));
      lines.push("");
    }
    d.sections?.forEach(sec=>{
      ...
      sec.items?.forEach(item=>{
        if(item.type==="job"){
          ...
          lines.push(`${item.degree}  ${item.institution||""}  ${item.date||""}`);
        }else{
    ...
    if(d.keyUpgrades?.length){
      lines.push("KEY UPGRADES");
      d.keyUpgrades.forEach(u=>lines.push(`• ${u}`));
    }
    return lines.join("\n");
  };
  ...
  const downloadPDF=async()=>{
    ...
    try{
      ...
      const{PDFDocument,rgb,StandardFonts}=window.PDFLib;
      const T=TEMPLATES.find(t=>t.id===template)||TEMPLATES[1];
      const S=T.styles;
      const doc=await PDFDocument.create();
      const F=await doc.embedFont(StandardFonts.Helvetica);
      const B=await doc.embedFont(StandardFonts.HelveticaBold);
      const AC=rgb(...S.accentColor);
      const NC=rgb(...S.nameColor);
      const HC=rgb(...S.headerColor);
      const BK=rgb(0.08,0.08,0.08);
      const GR=rgb(0.42,0.42,0.42);
      const PW=595,PH=842,ML=50,MR=50,MT=52,MB=48,CW=PW-ML-MR;
      let pg=doc.addPage([PW,PH]);let y=PH-MT;
      const np=()=>{pg=doc.addPage([PW,PH]);y=PH-MT;};
      const chk=(h)=>{if(y-h<MB)np();};
      const draw=(t,f,sz,x,clr)=>pg.drawText(t,{x,y,size:sz,font:f,color:clr});
      const wrap=(txt,f,sz,x,mw,lh,clr)=>{
        const words=cl(txt).split(/\s+/).filter(Boolean);
        if(!words.length)return;
        let line="";
        for(const w of words){
          const t=line?line+" "+w:w;
          // Trailing space on every drawn line is deliberate: pdf-lib draws each
          // line as a separate, independently-positioned text object with no
          // inherent word-boundary marker between it and whatever gets drawn
          // next (a date, a bullet, the next section). Many text extractors —
          // including plenty of real ATS resume parsers — just read text
          // objects in draw order and concatenate them raw, so two adjacent
          // objects with no embedded space merge into one glued-together word
          // (e.g. "PAJan 2024"). A harmless trailing space guards every seam.
          if(f.widthOfTextAtSize(t,sz)>mw&&line){chk(lh);draw(line+" ",f,sz,x,clr);y-=lh;line=w;}
          else line=t;
        }
        if(line){chk(lh);draw(line+" ",f,sz,x,clr);y-=lh;}
      };
      const rule=(thick,clr)=>{pg.drawLine({start:{x:ML,y:y+2},end:{x:PW-MR,y:y+2},thickness:thick,color:clr});};
      // Draws a bulleted paragraph as a single text call for its first line
      // (bullet + first words together) instead of two separate drawText
      // calls. This matters: pdf-lib silently drops leading/trailing spaces
      // on a text call, so two adjacent calls ("•" then "Calculate...") can
      // end up glued with zero separating space by some text extractors.
      // A space INSIDE one string always survives, so merging the bullet
      // into the first line closes that gap for good.
      const wrapBullet=(text,f,sz,lh,clr)=>{
        const words=cl(text).split(/\s+/).filter(Boolean);
        if(!words.length)return;
        const mw=CW-16;
        let line="";let firstLine=true;
        const flush=()=>{
          if(!line)return;
          chk(lh);
          const content=firstLine?("\u2022 "+line):line;
          const xPos=firstLine?(ML+6):(ML+16);
          pg.drawText(content,{x:xPos,y,size:sz,font:f,color:clr});
          y-=lh;firstLine=false;
        };
        for(const w of words){
          const t=line?line+" "+w:w;
          if(f.widthOfTextAtSize(t,sz)>mw&&line){flush();line=w;}
          else line=t;
        }
        flush();
        y-=2; // deliberately distinct from the uniform intra-bullet line height —
              // a gap identical to the normal line spacing can occasionally read
              // as "still the same flowing paragraph" to some text extractors,
              // which is what let a bullet symbol glue onto the previous bullet's
              // last word with no space. A visibly different gap size removes
              // that ambiguity.
      };

      if(isStructured){
        // STRUCTURED JSON → Perfect PDF
        const d=data;
        const drawCentered=(t,f,sz,clr)=>{
          const txt=cl(t);
          if(!txt)return;
          const w=f.widthOfTextAtSize(txt,sz); // width computed BEFORE the guard space so centering is unaffected
          const x=ML+Math.max(0,(CW-w)/2);
          chk(sz+4);
          pg.drawText(txt+" ",{x,y,size:sz,font:f,color:clr});
          y-=(sz+4);
        };
        // Name — centered header
        chk(S.nameSize+18);
        drawCentered(d.name,B,S.nameSize,NC);
        // Contact — centered, smaller, muted
        if(d.contact)drawCentered(d.contact,F,9,GR);
        y-=4;
        rule(S.ruleThick*1.5,AC);y-=12;
        // Sections
        d.sections?.forEach(sec=>{
          y-=8;chk(26); // reserve room so headers don't orphan at page bottom
          rule(S.ruleThick,AC);y-=10;
          wrap(cl(sec.heading),S.headerBold?B:F,9.5,ML,CW,14,HC);y-=4;
          const isSkillsSection=/SKILL|COMPETENC|TOOLS|SYSTEMS/i.test(sec.heading||"");
          sec.items?.forEach(item=>{
            if(item.type==="job"){
              y-=6;chk(34); // reserve room for title+company line before starting
              const ds=cl(item.date||"");
              const dw=ds?F.widthOfTextAtSize(ds,9):0;
              const fullTitle=cl(item.title||"");
              // Split "Job Title | Company – Location" so the role and the
              // company/location render as two distinct, readable lines
              // instead of one long bold run-on next to the date.
              const pipeIdx=fullTitle.indexOf("|");
              const rolePart=pipeIdx>-1?fullTitle.slice(0,pipeIdx).trim():fullTitle;
              const companyPart=pipeIdx>-1?fullTitle.slice(pipeIdx+1).replace(/^\|+/,"").trim():"";
              const ry=y;
              wrap(rolePart,B,10.5,ML,dw?CW-dw-12:CW,15,BK);
              if(ds)pg.drawText(ds+" ",{x:PW-MR-dw,y:ry,size:9,font:F,color:GR}); // trailing space: guards the seam before the company line / next text object
              if(companyPart){chk(13);wrap(companyPart,F,9.5,ML,CW,13,AC);}
              y-=3;
              item.bullets?.forEach(b=>{
                wrapBullet(b,F,9.5,13.5,BK);
              });
              y-=6; // breathing room before the next entry
            }else if(item.type==="edu"){
              chk(24);
              const inst=cl(item.institution||"");
              const date=item.date?` — ${cl(item.date)}`:"";
              const instTxt=inst+date;
              const instW=instTxt?F.widthOfTextAtSize(instTxt,9):0;
              const ry=y;
              wrap(cl(item.degree||""),B,9.5,ML,instTxt?CW-instW-10:CW,14,BK);
              if(instTxt)pg.drawText(instTxt+" ",{x:PW-MR-instW,y:ry,size:9,font:F,color:GR});
              y-=5;
            }else if(item.type==="bullet"){
              wrapBullet(item.text||"",F,9.5,13.5,BK);
            }else{
              chk(14);
              const bodyText=cl(item.text||"");
              if(isSkillsSection&&bodyText){
                // Render skills as a scannable, bullet-separated list instead
                // of one dense run-on paragraph.
                const parts=bodyText.split(/,|;|\u2022|\n/).map(s=>s.trim()).filter(Boolean);
                wrap(parts.length>3?parts.join("   \u2022   "):bodyText,F,9.5,ML,CW,15,BK);
              }else{
                wrap(bodyText,F,9.5,ML,CW,14,BK);
              }
              y-=2;
            }
          });
        ...
        // FALLBACK: Raw text renderer
        const DATE_RE=/((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\w*\.?\s+\d{4}|\d{1,2}\/\d{4}|\d{4})\s*(to|-|–|—)\s*((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\w*\.?\s+\d{4}|\d{1,2}\/\d{4}|\d{4}|Current|Present)\s*$/i;
        const HDRS=["PROFESSIONAL SUMMARY","SUMMARY","CORE COMPETENCIES","KEY COMPETENCIES","SKILLS","TECHNICAL SKILLS","SYSTEMS & TOOLS","PROFESSIONAL EXPERIENCE","WORK EXPERIENCE","EXPERIENCE","EMPLOYMENT","EARLY CAREER","EDUCATION & CERTIFICATIONS","EDUCATION","CERTIFICATIONS","ACHIEVEMENTS","AWARDS","REFERENCES","ADDITIONAL","TITLE TRANSLATIONS","KEY UPGRADES"];
        const lines=rawText.split("\n").map(l=>l.trim()).filter(Boolean);
        let idx=0,pt="";
        for(const line of lines){
          const UP=line.toUpperCase().replace(/[^A-Z &]/g,"").trim();
          const isHdr=HDRS.some(h=>UP===h||UP.startsWith(h));
          const dm=line.match(DATE_RE);
          const isJob=!isHdr&&!!dm&&line.length<300;
          const isBul=!isJob&&!isHdr&&(line.startsWith("•")||line.startsWith("-")&&line.length>4||line.startsWith("*")&&line.length>4);
          if(idx===0){chk(30);wrap(line,B,S.nameSize,ML,CW,S.nameSize+5,NC);rule(S.ruleThick*1.5,AC);y-=8;idx++;pt="name";continue;}
          if(idx<=2&&!isHdr&&(line.includes("@")||line.match(/\d{3}[\.\-]\d{3}/)||line.toLowerCase().includes("linkedin"))){chk(13);wrap(line,F,8.5,ML,CW,13,GR);idx++;pt="contact";continue;}
          if(isHdr){y-=10;chk(22);rule(S.ruleThick,AC);y-=9;wrap(UP,S.headerBold?B:F,9.5,ML,CW,14,HC);y-=3;idx++;pt="hdr";continue;}
          if(isJob){y-=(pt==="hdr")?4:9;chk(20);const ds=cl(dm[0].trim());const dw=F.widthOfTextAtSize(ds,9);const tp=cl(line.slice(0,line.search(DATE_RE)).replace(/[\s,]+$/,""));const ry=y;wrap(tp,B,10,ML,CW-dw-12,16,BK);pg.drawText(ds+" ",{x:PW-MR-dw,y:ry,size:9,font:F,color:GR});if(y===ry)y-=16;idx++;pt="job";continue;}
          if(isBul){wrapBullet(line.replace(/^[•*\-]\s*/,""),F,9.5,14,BK);idx++;pt="bul";continue;}
          chk(14);wrap(line,F,9.5,ML,CW,14,BK);idx++;pt="body";
        }
      }
      ...
      const url=URL.createObjectURL(blob);
      const a=document.createElement("a");
      a.href=url;a.download=base+".pdf";a.style.display="none";
      document.body.appendChild(a);a.click();
      setTimeout(()=>{try{document.body.removeChild(a);}catch{}URL.revokeObjectURL(url);},2000);
      setPdfState("done");setTimeout(()=>setPdfState("idle"),3000);
    }catch(e){console.error(e);setPdfState("idle");setErr("PDF error: "+e.message);}
  };

  const copyForWord=async()=>{
    const txt=isStructured?toPlainText(data):(rawText||"");
    if(!txt){setErr("No text to copy.");return;}
    try{
      await navigator.clipboard.writeText(txt);
      setCopied(true);setTimeout(()=>setCopied(false),4000);
    }catch(e){setErr("Copy failed. Please select and copy the text manually.");}
  };
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          {TEMPLATES.map(t=>(
            <div key={t.id} onClick={()=>setTemplate(t.id)}
              ...
              <div style={{fontSize:10,color:C.gray}}>{t.desc}</div>
            </div>
          ))}
        </div>
      </div>
      <button className="btn btn-gold" style={{width:"100%",justifyContent:"center",marginBottom:10}}
        onClick={downloadPDF} disabled={pdfState==="loading"}>
        {pdfState==="loading"?"⏳ Generating PDF...":pdfState==="done"?"✅ PDF Downloaded!":"📄 Download PDF (ATS-Safe)"}
      </button>
      <div style={{background:"rgba(0,200,150,0.06)",border:"1px solid rgba(0,200,150,0.15)",borderRadius:10,padding:"12px 14px"}}>
        <div style={{fontSize:12,fontWeight:700,color:C.verified,marginBottom:3}}>📋 For Word / Google Docs</div>
        <div style={{fontSize:11,color:C.gray,marginBottom:8}}>Tap Copy → open Google Docs → New Document → paste. Done.</div>
        <button className="btn btn-green" style={{width:"100%",justifyContent:"center"}} onClick={copyForWord}>
          {copied?"✅ Copied! Open Google Docs and paste →":"📋 Copy Resume Text"}
        </button>
      </div>
      {err&&<div style={{marginTop:8,fontSize:11,color:C.red,background:"rgba(231,76,60,0.08)",borderRadius:8,padding:"8px 12px",border:"1px solid rgba(231,76,60,0.2)"}}>{err}</div>}
    </div>
  );
}
  ...
  return(
    <div className="panel">
      ...
      <div className="panel-body">
        ...
        {!loading&&!roles.length&&raw&&<div className="ai-response"><div className="ai-label">Roles</div><div className="ai-content">{tidyAIText(raw)}</div></div>}
      </div>
    </div>
  );
}
  ...
  return(
    <div className="panel">
      ...
      <div className="panel-body">
        ...
        {ans&&!loading&&<div className="ai-response"><div className="ai-label">Culture Guide</div><div className="ai-content">{tidyAIText(ans)}</div></div>}
      </div>
    </div>
  );
}
        ...
        <div className="panel-body">
          ...
          {aiTipLoading?(
            ...
            <div className="ai-response">
              ...
              <div className="ai-content">{tidyAIText(aiTip)}</div>
            </div>
          )}
...
function ATSModule({country,onScore}){
  ...
  const [apiError,setApiError]=useState("");
  const [apiErrorHint,setApiErrorHint]=useState("");
  ...
  const reset=()=>{setStep(0);setResult(null);setResumeText("");setJobDesc("");setCompany("");setRole("");setActiveTab("optimized");setFileName("");setExtractError("");setApiError("");setApiErrorHint("");};
...
Analyze the resume against the job description and return ONLY a JSON object (no markdown):
{
  ...
  "optimizedResume": "full rewritten resume with missing keywords woven in naturally. Never fabricate experience.",
...
CRITICAL: Never fabricate. Weave keywords naturally. Use ${country.name} spelling/terminology.`;
    setApiError("");setApiErrorHint("");
    const res=await callClaude([{role:"user",content:`RESUME:\n${resumeText}\n\nJOB DESCRIPTION for ${role||"the role"} at ${company||"the company"}:\n${jobDesc}`}],sys,4000);
    if(!res&&lastClaudeError){
      // The API call itself failed (bad/missing key, no credits, network, rate limit) —
      // show that honestly instead of quietly faking a "successful" result
      // that's just the original resume echoed back with placeholder scores.
      setApiError(lastClaudeError);
      setApiErrorHint("This usually means the Claude API key isn't configured correctly for this deployment, the account is out of credits, or a network/CORS issue blocked the request.");
    ...
    try{
      ...
      const s=clean.indexOf("{");const e=clean.lastIndexOf("}")+1;
      const parsed=JSON.parse(clean.slice(s,e));
      parsed.optimizedResume=tidyAIText(parsed.optimizedResume);
      setResult(parsed);setStep(2);
      onScore&&onScore(parsed.beforeScore,parsed.afterScore,role,company);
    }catch{
      setApiError("The AI response couldn't be read as a resume.");
      setApiErrorHint("This usually happens when the response gets cut off before it finishes (a long resume can need more room than the request allowed), or when the job description field is empty or not real job-posting text. Double-check the job description below, then try again.");
      setLoading(false);setStep(0);return;
    }
    setLoading(false);
  };
    ...
    <div className="panel">
      ...
      <div className="panel-body">
        {apiError&&(
          <div style={{background:"rgba(231,76,60,0.1)",border:"1px solid rgba(231,76,60,0.3)",borderRadius:10,padding:"14px 18px",marginBottom:16,fontSize:13,color:"#E74C3C",lineHeight:1.7}}>
            ⚠️ <strong>The AI request failed:</strong> {apiError}
            {apiErrorHint&&<div style={{marginTop:6,fontSize:12,color:"rgba(231,76,60,0.85)"}}>{apiErrorHint}</div>}
          </div>
        )}
      ...
      {activeTab==="optimized"&&(
        <div>
          ...
          <div className="ai-response">
            ...
            <div className="ai-content">{tidyAIText(result.optimizedResume)}</div>
          </div>
          <PDFDownloadButton data={result.optimizedResume} filename={`CrossBorder-ATS-${role||"Resume"}.pdf`}/>
