...
function ResumeModule({country,onComplete}){
  ...
  const extractTextFromFile=async(f)=>{
    ...
    try{
      let text="";
      if(ext==="pdf"){
        setExtractError("PDF upload not supported — please save your CV as Word (.docx) and upload that instead, or use the Paste Text tab.");
            ...
            {inputMode==="upload"&&(
              <div>
                <div style={{background:"rgba(0,200,150,0.06)",border:"1px solid rgba(0,200,150,0.15)",borderRadius:10,padding:"12px 16px",marginBottom:14,fontSize:12,color:C.cream,lineHeight:1.7}}>
                  ✅ <strong style={{color:C.verified}}>Word (.docx) and plain text (.txt)</strong> all supported. We extract the text automatically.
                </div>
                <div className="upload-zone"
                  ...
                  {extracting?(
                    ...
                    <div>
                      ...
                      <div className="upload-title">Drop your Word CV here</div>
                      <div className="upload-sub">or tap to browse files</div>
                      <div className="upload-formats">DOCX · DOC · TXT</div>
                    </div>
                  )}
                </div>
                {extractError&&(
...
function preprocessResume(raw){
  ...
  const sortedH=[...HEADERS].sort((a,b)=>b.length-a.length);

  const DATE_RE=/((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s+\d{4}\s*[–—\-]+\s*(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s+\d{4}|(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s+\d{4}\s*[–—\-]+\s*Present|\d{4}\s*[–—\-]+\s*(?:Present|\d{4}))/i;
  const DATE_END=new RegExp(DATE_RE.source+"\\s*$","i");
  const DEGREE_RE=/^(B\.?Tech|B\.?Sc|M\.?Sc|MBA|Ph\.?D|Bachelor|Master|Doctor|Certificate|Diploma|Cybersecurity|Evolve|CFA)/i;
  const INST_RE=/^(Ladoke|Google|Evolve|University|College|Institute|School|Academy|CFA|Microsoft|Amazon)/i;

  // ── PHASE 1: Flatten and re-split intelligently ──
  // Convert all line endings to spaces, then re-insert newlines at known boundaries
  let flat=raw.replace(/\r\n|\r/g,"\n");

  // First protect section headers — surround each with double newlines
  for(const h of sortedH){
    const esc=h.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");
    flat=flat.replace(new RegExp(`(^|\\s)(${esc})(\\s|$)`,"gim"),"\n\n$2\n\n");
  }

  // Split at bullet markers
  flat=flat.replace(/[ \t]*[•·▪][ \t]*/g,"\n• ");

  // Now process line by line — join continuation lines
  ...
  for(let i=0;i<rawLines.length;i++){
    ...
    const isHdr=sortedH.some(h=>UP===h.toUpperCase()||UP.startsWith(h.toUpperCase()+"—")||UP.startsWith(h.toUpperCase()+" —"));
    const isBul=l.startsWith("•");
    ...
    if(isBul){
      if(buf){joined.push(buf);buf="";}
      buf=l;
      // Check if next non-empty line should be joined to this bullet
      continue;
    }

    // If this line has a pipe (job title fragment) or date, try to join with buffer
    if(buf){
      const bufHasPipe=buf.includes("|")||buf.includes("–")||buf.includes("—");
      const bufIsBul=buf.startsWith("•");
      const lineIsDate=DATE_RE.test(l)&&!l.match(/[a-z].*[A-Z]/);

      // Join if: buffer is a bullet and line continues it (no pipe, no date)
      if(bufIsBul&&!hasDate&&!hasPipe&&!isHdr){
        buf=buf+" "+l;continue;
      }
      // Join if: buffer is partial job title (no date yet) and line has the date or rest of title
      if(bufHasPipe&&!buf.match(DATE_END)&&!bufIsBul){
        buf=buf+" "+l;continue;
      }
      // Otherwise flush buffer and start new
      joined.push(buf);buf=l;
    }else{
      buf=l;
    }
  }
  ...
  // ── PHASE 2: Convert joined lines to markers ──
  const out=[];
  let lineIdx=0;
  ...
  for(let i=0;i<joined.length;i++){
    ...
    const isHdr=sortedH.some(h=>UP===h.toUpperCase()||UP===h.toUpperCase()+" EXPERIENCE"||UP.replace(/\s*—\s*/g," — ")===h.toUpperCase());
    const isBul=line.startsWith("•")||line.startsWith("-");
    const dm=line.match(DATE_END);
    const hasSep=line.includes("|")||line.includes("–")||line.includes("—");
    const isJob=!isHdr&&hasSep&&line.match(/\d{4}/)&&line.length<250;
    const isDeg=!isHdr&&!isJob&&DEGREE_RE.test(line);
    const isInst=!isHdr&&!isJob&&!isDeg&&INST_RE.test(line)&&line.length<80;

    // NAME (first real line)
    if(lineIdx===0){
      // Split name from contact if on same line
      const m=line.match(/^([A-Z][A-Z\s]+?)\s+(?=\d|[A-Za-z]+,\s+[A-Z]{2})/);
      if(m){
        out.push("###NAME: "+m[1].trim());
        out.push("###CONTACT: "+line.slice(m[0].length).trim());
      }else{
        out.push("###NAME: "+line);
      }
      lineIdx++;continue;
    ...
    // Contact lines (early lines with @ or phone)
    if(lineIdx<=3&&(line.includes("@")||line.match(/\d{3}[\s\-\.]\d{3}/)||line.toLowerCase().includes("linkedin"))){
      out.push("###CONTACT: "+line);lineIdx++;continue;
    }

    if(isHdr){out.push("###SECTION: "+UP.replace(/\s*—\s*/g," — "));lineIdx++;continue;}

    if(isJob){
      if(dm){
        const ds=dm[0].trim();
        const tp=line.slice(0,line.search(DATE_END)).trim().replace(/[\s,]+$/,"");
        out.push("###JOB: "+tp+" ||| "+ds);
      }else{
        out.push("###JOB: "+line+" ||| ");
      }
    ...
    if(isBul){out.push("###BULLET: "+line.replace(/^[•\-]\s*/,""));lineIdx++;continue;}

    if(isDeg){
      // Look ahead for institution on next line
      const nxt=joined[i+1]?.trim()||"";
      if(INST_RE.test(nxt)&&nxt.length<80){
        out.push("###EDU: "+line+" ||| "+nxt);
        i++;// skip next
      }else{
        // Try to split inline: "B.Tech ... Ladoke Akintola"
        const instMatch=line.match(/^(.+?)\s{2,}((?:Ladoke|Google|Evolve|University|College|Institute|School|Academy|CFA|Microsoft).+)$/);
        if(instMatch){
          out.push("###EDU: "+instMatch[1].trim()+" ||| "+instMatch[2].trim());
        }else{
          out.push("###EDU: "+line+" ||| ");
        }
      }
...
async function generateAndDownloadPDF(text, filename="CrossBorder-CV.pdf", templateId="modern"){
  try{
    ...
    // Use raw text directly — parse line by line
    const rawText=text||"";
    const hasMarkers=rawText.includes("###NAME:")||rawText.includes("###SECTION:");
    const lines=rawText.split("\n").map(l=>l.trim()).filter(Boolean);
    let prevType="";
    const KNOWN_HDRS=["PROFESSIONAL SUMMARY","CORE COMPETENCIES","PROFESSIONAL EXPERIENCE","WORK EXPERIENCE","EARLY CAREER","EDUCATION & CERTIFICATIONS","EDUCATION","CERTIFICATIONS","SKILLS","ACHIEVEMENTS","REFERENCES","ADDITIONAL","SYSTEMS & TOOLS"];
    const DATE_RE_END=/((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s+\d{4}\s*[–—\-]+\s*(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s+\d{4}|(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s+\d{4}\s*[–—\-]+\s*Present|\d{4}\s*[–—\-]+\s*(?:Present|\d{4}))\s*$/i;
    let lineIdx=0;
...
async function generateAndDownloadWord(text, filename="CrossBorder-CV.docx"){
  try{
    await new Promise((resolve,reject)=>{
      ...
      const s=document.createElement("script");
      s.src="https://unpkg.com/docx@8.2.3/build/index.js";
      s.onload=resolve;
      s.onerror=()=>{
        const s2=document.createElement("script");
        s2.src="https://cdn.jsdelivr.net/npm/docx@8.2.3/build/index.js";
    ...
    const {Document,Packer,Paragraph,TextRun,BorderStyle}=window.docx;
      const convertInchesToTwip=(n)=>Math.round(n*1440);
    const NAVY="0D1B2A",GOLD="C8881A",GRAY="666666";
    ...
    for(const line of lines){
      if(line.startsWith("###NAME:")){
        ...
        children.push(new Paragraph({
          children:[new TextRun({text:n,bold:true,size:34,color:NAVY,font:"Calibri"})],
          spacing:{after:60},
          border:{bottom:{color:GOLD,style:BorderStyle.SINGLE,size:8,space:4}},
        }));
      }else if(line.startsWith("###CONTACT:")){
        const c=line.replace("###CONTACT:","").trim();
        children.push(new Paragraph({
          children:[new TextRun({text:c,size:18,color:GRAY,font:"Calibri"})],
          spacing:{after:100},
        }));
      }else if(line.startsWith("###SECTION:")){
        const h=line.replace("###SECTION:","").trim();
        children.push(new Paragraph({
          children:[new TextRun({text:h,bold:true,size:20,color:GOLD,font:"Calibri",allCaps:true})],
          spacing:{before:200,after:60},
          border:{bottom:{color:GOLD,style:BorderStyle.SINGLE,size:4,space:1}},
        }));
      }else if(line.startsWith("###JOB:")){
        const j=line.replace("###JOB:","").trim();
        const sep=j.indexOf("|||");
        const title=sep>=0?j.slice(0,sep).trim():j;
        const date=sep>=0?j.slice(sep+3).trim():"";
        children.push(new Paragraph({
          children:[
            new TextRun({text:title,bold:true,size:22,color:NAVY,font:"Calibri"}),
            ...(date?[new TextRun({text:"\t"+date,size:18,color:GRAY,font:"Calibri"})]:[]),
          ],
        ...
        children.push(new Paragraph({
          children:[new TextRun({text:b,size:20,font:"Calibri",color:"111111"})],
          bullet:{level:0},
          spacing:{after:40},
        }));
      }else if(line.startsWith("###BODY:")){
        const b=line.replace("###BODY:","").trim();
        children.push(new Paragraph({
          children:[new TextRun({text:b,size:20,font:"Calibri",color:"111111"})],
          spacing:{after:60},
        }));
      }else if(line.startsWith("###EDU:")){
        const e=line.replace("###EDU:","").trim();
        const sep=e.indexOf("|||");
        const deg=sep>=0?e.slice(0,sep).trim():e;
        const inst=sep>=0?e.slice(sep+3).trim():"";
        children.push(new Paragraph({
          children:[
            new TextRun({text:deg,bold:true,size:20,font:"Calibri"}),
            ...(inst?[new TextRun({text:"\t"+inst,size:18,color:GRAY,italics:true,font:"Calibri"})]:[]),
          ],
          spacing:{after:40},
          tabStops:[{type:"right",position:convertInchesToTwip(6.5)}],
        }));
...
function PDFDownloadButton({text,filename}){
  ...
  const downloadPDF=async()=>{
    if(!text){setStatus("error");setMsg("No resume text.");return;}
    setStatus("loading");setMsg("");
    try{
      // Load pdf-lib with timeout
      if(!window.PDFLib){
        await new Promise((res,rej)=>{
          const timeout=setTimeout(()=>rej(new Error("pdf-lib load timeout")),15000);
          const s=document.createElement("script");
          s.src="https://cdn.jsdelivr.net/npm/pdf-lib@1.17.1/dist/pdf-lib.min.js";
          s.onload=()=>{clearTimeout(timeout);res();};
          s.onerror=()=>{clearTimeout(timeout);rej(new Error("pdf-lib failed to load"));};
          document.head.appendChild(s);
        });
      }
      const {PDFDocument,rgb,StandardFonts}=window.PDFLib;
      const tmpl=TEMPLATES.find(t=>t.id===template)||TEMPLATES[1];
      const S=tmpl.styles;
      ...
      let page=doc.addPage([PW,PH]);
      let y=PH-MT;
      const np=()=>{page=doc.addPage([PW,PH]);y=PH-MT;};
      const chk=(h)=>{if(y-h<MB)np();};
      const bk=(txt,f,sz,x,mw,lh,clr)=>{
        if(!txt||!txt.trim())return;
        const words=String(txt).split(/\s+/);
        let cur="";
        for(const w of words){
          const t=cur?cur+" "+w:w;
          if(f.widthOfTextAtSize(t,sz)>mw&&cur){chk(lh);page.drawText(cur,{x,y,size:sz,font:f,color:clr});y-=lh;cur=w;}
          else cur=t;
        }
        if(cur){chk(lh);page.drawText(cur,{x,y,size:sz,font:f,color:clr});y-=lh;}
      };

      const HDRS=["PROFESSIONAL SUMMARY","CORE COMPETENCIES","KEY COMPETENCIES","PROFESSIONAL EXPERIENCE","WORK EXPERIENCE","EMPLOYMENT","EARLY CAREER — EMERGING MARKETS EXPERIENCE","EARLY CAREER - EMERGING MARKETS EXPERIENCE","EARLY CAREER","EDUCATION & CERTIFICATIONS","EDUCATION AND CERTIFICATIONS","EDUCATION","CERTIFICATIONS","TECHNICAL SKILLS","SKILLS","SYSTEMS & TOOLS","SYSTEMS AND TOOLS","ACHIEVEMENTS","AWARDS","REFERENCES","ADDITIONAL INFORMATION","ADDITIONAL","LANGUAGES","VOLUNTEER","PROJECTS","TITLE TRANSLATIONS MADE","KEY UPGRADES"];
      ...
      // Sanitize special chars that WinAnsi can't encode
      const sanitize=(s)=>(s||"").replace(/[━─═]/g,"—").replace(/[•·▪▸]/g,"•").replace(/[""]/g,'"').replace(/['']/g,"'").replace(/[…]/g,"...").replace(/[^ -•–—]/g,"");
      ...
      for(const line of lines){
        if(!line)continue;
        const UP=line.toUpperCase().trim();
        const isHdr=HDRS.some(h=>UP===h||UP.startsWith(h+"—")||UP.startsWith(h+" —"));
        const isBul=line.startsWith("•")||(/^-\s/.test(line));
        const dm=line.match(DTRE);
        const hasSep=line.includes("|")||line.includes("–")||line.includes("—");
        const isJob=!isHdr&&hasSep&&!!line.match(/\d{4}/)&&line.length<200;

        if(idx===0){
          chk(28);bk(line,bold,S.nameSize,ML,CW,S.nameSize+4,NAME_C);
          page.drawLine({start:{x:ML,y:y+2},end:{x:PW-MR,y:y+2},thickness:S.ruleThick*1.5,color:ACCENT});
          y-=6;idx++;pt="name";continue;
        }
        if(idx<=3&&(line.includes("@")||/\d{3}[\s\-]\d{3}/.test(line)||line.toLowerCase().includes("linkedin")||line.includes("• "))){
          chk(13);bk(line,font,8.5,ML,CW,13,GRAY);idx++;pt="contact";continue;
        }
        if(isHdr){
          y-=(pt==="name"||pt==="contact")?6:12;chk(22);
          page.drawLine({start:{x:ML,y:y+3},end:{x:PW-MR,y:y+3},thickness:S.ruleThick,color:ACCENT});
          y-=9;bk(UP,S.headerBold?bold:font,9.5,ML,CW,14,HDR_C);y-=2;idx++;pt="hdr";continue;
        }
        if(isJob){
          y-=(pt==="hdr")?4:10;chk(20);
          if(dm){
            const ds=dm[0].trim();
            const tp=line.slice(0,line.search(DTRE)).trim().replace(/[,\s]+$/,"");
            const dw=font.widthOfTextAtSize(ds,9);
            const ry=y;
            bk(tp,bold,10,ML,CW-dw-12,16,BLACK);
            if(y!==ry){}else{page.drawText(ds,{x:PW-MR-dw,y:ry,size:9,font,color:GRAY});}
            if(y===ry)y-=16;
          }else{bk(line,bold,10,ML,CW,16,BLACK);}
          idx++;pt="job";continue;
        }
        if(isBul){
          const bt=line.replace(/^[•\-]\s*/,"");
          chk(14);page.drawText("•",{x:ML+4,y,size:8.5,font,color:GRAY});
          bk(bt,font,9.5,ML+16,CW-16,14,BLACK);y-=1;idx++;pt="bul";continue;
        }
        chk(14);bk(line,font,9.5,ML,CW,14,BLACK);idx++;pt="body";
      }
      ...
      const url=URL.createObjectURL(blob);
      const a=document.createElement("a");
      a.href=url;a.download=baseName+".pdf";a.style.display="none";
      document.body.appendChild(a);a.click();
      ...
      setStatus("error");setMsg("PDF failed: "+e.message);
    }
  };

    const downloadWord=async()=>{
    if(!text){setStatus("error");setMsg("No text.");return;}
    setStatus("loading");setMsg("word");
    try{
      const san=(s)=>(s||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/[━─═━─]/g,"-").replace(/[“”]/g,'"').replace(/[‘’]/g,"'").replace(/…/g,"...").replace(/[^	
 -퟿]/g,"");

      const HDRS=["PROFESSIONAL SUMMARY","CORE COMPETENCIES","KEY COMPETENCIES","PROFESSIONAL EXPERIENCE","WORK EXPERIENCE","EARLY CAREER","EDUCATION & CERTIFICATIONS","EDUCATION","CERTIFICATIONS","SKILLS","SYSTEMS & TOOLS","ACHIEVEMENTS","REFERENCES","ADDITIONAL","TITLE TRANSLATIONS","KEY UPGRADES"];
      const DTRE=/((?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\w*\.?\s+\d{4}\s*[-–—]+\s*(?:(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\w*\.?\s+\d{4}|Present))\s*$/i;
...
").map(l=>l.trim()).filter(Boolean);
      ...
      const run=(t,opts={})=>`<w:r><w:rPr>${opts.b?"<w:b/>":""}${opts.color?`<w:color w:val="${opts.color}"/>`:""}${opts.i?"<w:i/>":""}${opts.caps?"<w:caps/>":""}<w:sz w:val="${opts.sz||20}"/><w:szCs w:val="${opts.sz||20}"/><w:rFonts w:ascii="Calibri" w:hAnsi="Calibri"/></w:rPr><w:t xml:space="preserve">${t}</w:t></w:r>`;

      const para=(runs,opts={})=>`<w:p><w:pPr><w:spacing w:before="${opts.before||0}" w:after="${opts.after||80}"/>${opts.border?'<w:pBdr><w:bottom w:val="single" w:sz="4" w:space="1" w:color="C8881A"/></w:pBdr>':""} ${opts.indent?`<w:ind w:left="${opts.indent}" w:hanging="${opts.indent}"/>`:""} ${opts.bullet?'<w:numPr><w:ilvl w:val="0"/><w:numId w:val="1"/></w:numPr>':""}</w:pPr>${runs}</w:p>`;
      ...
      for(const line of lines){
        ...
        const isBul=line.startsWith("•")||line.startsWith("- ");
        const dm=line.match(DTRE);
        const hasSep=line.includes("|")||/[-–—]/.test(line);
        const isJob=!isHdr&&hasSep&&/\d{4}/.test(line)&&line.length<220;

        if(idx===0){
          body+=para(run(s,{b:true,sz:34,color:"0D1B2A"}),{after:60,border:true,before:0});
          idx++;continue;
        }
        if(idx<=3&&(line.includes("@")||/\d{3}/.test(line)||line.toLowerCase().includes("linkedin"))){
          body+=para(run(s,{sz:18,color:"666666"}),{after:100});
          idx++;continue;
        }
        if(isHdr){
          body+=para(run(san(UP),{b:true,sz:20,color:"C8881A",caps:true}),{before:240,after:60,border:true});
          idx++;continue;
        }
        if(isJob){
          const ds=dm?san(dm[0].trim()):"";
          const tp=ds?san(line.slice(0,line.search(DTRE)).trim()):s;
          body+=para(
            run(tp,{b:true,sz:22,color:"0D1B2A"})+(ds?run("	"+ds,{sz:18,color:"666666"}):""),
            {before:120,after:40}
          );
          idx++;continue;
        }
        if(isBul){
          body+=para(run(san(line.replace(/^[•\-]\s*/,"")),{sz:20}),{after:40,bullet:true,indent:360});
          idx++;continue;
        }
        body+=para(run(s,{sz:20}),{after:40});
        idx++;
      }
      ...
      const files={
        "word/document.xml":`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body>${body}<w:sectPr><w:pgMar w:top="1080" w:right="1080" w:bottom="1080" w:left="1080"/></w:sectPr></w:body></w:document>`,
        ...
        "word/_rels/document.xml.rels":`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/numbering" Target="numbering.xml"/></Relationships>`,
      };

      // Build zip using browser's CompressionStream API
      const crc32=(buf)=>{
        let c=0xFFFFFFFF;
        const t=new Uint8Array(buf);
        const table=[];for(let i=0;i<256;i++){let v=i;for(let j=0;j<8;j++)v=v&1?0xEDB88320^(v>>>1):v>>>1;table[i]=v;}
        for(const b of t)c=table[(c^b)&0xFF]^(c>>>8);
      ...
      const cat=(...a)=>{const t=a.reduce((s,x)=>s+x.length,0);const r=new Uint8Array(t);let o=0;for(const x of a){r.set(x,o);o+=x.length;}return r;};
      ...
      for(const[name,content] of Object.entries(files)){
        ...
        const localHdr=cat(
          ...
          u16(20),u16(0),u16(0),// version,flag,method(store)
          u16(0),u16(0),// time,date
          u32(crc),u32(dataB.length),u32(dataB.length),
          u16(nameB.length),u16(0),
          nameB,dataB
        );
        entries.push({nameB,dataB,crc,offset,localHdr});
      ...
      const zipBytes=cat(...entries.map(e=>e.localHdr),cd,eocd);
      const blob=new Blob([zipBytes],{type:"application/vnd.openxmlformats-officedocument.wordprocessingml.document"});
      const url=URL.createObjectURL(blob);
      const a=document.createElement("a");
      a.href=url;a.download=baseName+".docx";a.style.display="none";
      document.body.appendChild(a);a.click();
      setTimeout(()=>{try{document.body.removeChild(a);}catch{}URL.revokeObjectURL(url);},2000);
      setStatus("done");setTimeout(()=>setStatus("idle"),3000);
    }catch(e){
      console.error("Word error:",e);
      setStatus("error");setMsg("Word failed: "+e.message);
    }
  };
    ...
    <div style={{marginTop:16,background:"rgba(0,0,0,0.15)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:14,padding:"18px 20px"}}>
      ...
      <div style={{display:"flex",gap:9,flexWrap:"wrap"}}>
        <button className="btn btn-gold" style={{flex:1,justifyContent:"center"}}
          onClick={downloadPDF} disabled={status==="loading"}>
          {status==="loading"&&msg!==""?"⏳...":status==="done"?"✅ Done!":"📄 Download PDF"}
        </button>
        <button className="btn btn-ghost" style={{flex:1,justifyContent:"center"}}
          onClick={downloadWord} disabled={status==="loading"}>
          {status==="loading"&&msg==="word"?"⏳...":status==="done"?"✅ Done!":"📝 Download Word"}
        </button>
      </div>
      {status==="error"&&<div style={{marginTop:8,fontSize:11,color:C.red,background:"rgba(231,76,60,0.08)",borderRadius:8,padding:"8px 12px",border:"1px solid rgba(231,76,60,0.2)"}}>{msg}</div>}
      <div style={{fontSize:10,color:C.gray,marginTop:8}}>PDF is ATS-safe · Word opens in Google Docs or Microsoft Word</div>
    </div>
  );
}
            ...
            <div className="upload-zone" onClick={()=>!extracting&&fileRef.current.click()}
              ...
              {extracting?(
                ...
                <div>
                  ...
                  <div className="upload-title">{resumeText?`Loaded: ${fileName}`:"Drop your Word CV here"}</div>
